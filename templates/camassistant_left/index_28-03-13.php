...
/*$alert_sql ="SELECT count(*) from #__cam_vendor_occupational_license   WHERE OLN_expdate <= '".$today."' AND vendor_id=".$user->id; //validation to expdate of docs
		...
		$alert_sql ="SELECT count(*) from #__cam_vendor_liability_insurence   WHERE GLI_end_date  >= '".$today."' AND GLI_status = 1 AND vendor_id=".$user->id; //validation to expdate of docs
...
if($suspend == 'suspend'){
$display = 'none';
$message = "<div id='container_inner_suspend'>Your Account has been locked by a CAMassistant.com Administrator. Please call our office at 561-246-3830 so we can re-enable your account.</div>";
}
else{
$display = '';
}
...
<?php } ?>
			...
			<?php if($user->user_type == '11') : ?>
					...
					<?PHP  if(($vendor_GLI_compliance_alert['W9_exist'] != 0 && $vendor_GLI_compliance_alert['GLI_exist'] != 0 && $vendor_GLI_compliance_alert['GLI_status'] == 0 && $vendor_GLI_compliance_alert['W9_status'] == 0 && ($PLN_needed == 'no' || ($PLN_needed == 'yes' && $vendor_GLI_compliance_alert['PLN_status'] == 0))) && (($vendor_GLI_compliance_alert['GLI_exp'] == 0) && ($vendor_GLI_compliance_alert['W9_status'] == 0) &&  ($PLN_needed == 'no' || ($PLN_needed == 'yes' && $vendor_GLI_compliance_alert['PLN_exp'] == 0))) ) {  ?>
