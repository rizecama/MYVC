...
if($user->user_type == '11')
{
		...
		$alert_sql ="SELECT count(*) from #__cam_vendor_liability_insurence  WHERE GLI_status = 1 AND vendor_id=".$user->id; //validation to status of docs
		$db->Setquery($alert_sql);
		$res3 = $db->loadResult();
		if($res3 != 0) $res3=0; else $res3=1;
		$alert['GLI_status']= $res3;
		
		$alert_sql ="SELECT count(*) from #__cam_vendor_compliance_w9docs  WHERE w9_status = 0 AND vendor_id=".$user->id; //validation to status of docs
		$db->Setquery($alert_sql);
		$res4 = $db->loadResult();
...
<?php } ?>
			<?php if($user->user_type == '11') : ?>
					...
					<?PHP  if(($vendor_GLI_compliance_alert['W9_exist'] != 0 && $vendor_GLI_compliance_alert['GLI_exist'] != 0 && $vendor_GLI_compliance_alert['GLI_status'] == 0 && $vendor_GLI_compliance_alert['W9_status'] == 0 && ($PLN_needed == 'no' || ($PLN_needed == 'yes' && $vendor_GLI_compliance_alert['PLN_status'] == 0))) && (($vendor_GLI_compliance_alert['GLI_exp'] == 0) &&  ($PLN_needed == 'no' || ($PLN_needed == 'yes' && $vendor_GLI_compliance_alert['PLN_exp'] == 0))) ) {  ?>
