...
$after30day=date('Y-m-d', strtotime('-30 days', strtotime($today)));
//echo $after1day; exit;
...
LEFT JOIN #__cam_vendor_workers_companies_insurance as D ON D.vendor_id=u.id
 WHERE u.user_type = 11 AND ((A.OLN_expdate = '".$after30day."' AND A.OLN_expdate != '00-00-0000') OR (B.PLN_expdate = '".$after30day."' AND B.PLN_expdate != '00-00-0000') OR (C.GLI_end_date = '".$after30day."' AND C.GLI_end_date != '00-00-0000') OR (AI.aip_end_date = '".$after30day."' AND AI.aip_end_date != '00-00-0000') OR (WC.wc_end_date = '".$after30day."' AND WC.wc_end_date != '00-00-0000') OR (D.WCI_end_date = '".$after30day."' AND D.WCI_end_date != '00-00-0000'))  ORDER BY u.id ";
	...
	$user = JFactory::getUSer($vendor_ids[$n]);
        ...
        for($g=0; $g<count($gdate); $g++)
        {
		   ...
		   $datediff = $datediff / 30 ;
			   ...
			   if($daylen == $datediff || ( $gdate[$g]->GLI_end_date < $today && $gdate[$g]->GLI_end_date != '0000-00-00' ) ){
			   $sendgli = 1;
			   $gli = $g + 1 ;
        ...
        for($a=0; $a<count($aipdate); $a++)
        {
		   ...
		   $datediff = $datediff / 30 ;
			   if($daylen == $datediff || ( $aipdate[$a]->aip_end_date < $today && $aipdate[$a]->aip_end_date != '0000-00-00' ) ){
			   $sendaip = 1;
			   $aip = $a + 1 ;
        ...
        for($o=0; $o<count($oldate); $o++)
        {
		   ...
		   $datediff = $datediff / 30 ;
			   if($daylen == $datediff || ( $oldate[$o]->OLN_expdate < $today && $oldate[$o]->OLN_expdate != '0000-00-00' ) ){
			   $sendoln = 1;
			   $oln = $o + 1 ;
        ...
        for($wc=0; $wc<count($wcdate); $wc++)
        {
		   ...
		   $datediff = $datediff / 30 ;
			   if($daylen == $datediff || ( $wcdate[$wc]->wc_end_date < $today && $wcdate[$wc]->wc_end_date != '0000-00-00' ) ){
			   $sendwc = 1;
			   $wcd = $wc + 1 ;
        ...
        for($p=0; $p<count($pldate); $p++)
        {
		   ...
		   $datediff = $datediff / 30 ;
               if($daylen == $datediff || ( $pldate[$p]->PLN_expdate < $today && $pldate[$p]->PLN_expdate != '0000-00-00' ) ){
			   $sendpln = 1;
			   $pln = $p + 1 ;
        ...
        for($w=0; $w<count($wdate); $w++)
        {
		   ...
		   $datediff = $datediff / 30 ;
           	   if($daylen == $datediff || ( $wdate[$w]->WCI_end_date < $today && $wdate[$w]->WCI_end_date != '0000-00-00' ) ){
			   $sendwci = 1;
			   $wci = $w + 1 ;
        ...
        for($u=0; $u<count($udate); $u++)
        {
		   ...
		   $datediff = $datediff / 30 ;
           	   if($daylen == $datediff || ( $udate[$u]->UMB_expdate < $today && $udate[$u]->UMB_expdate != '0000-00-00' ) ){
			   $sendumb = 1;
			   $umb = $u + 1 ;
