		...
		echo "<pre>"; print_r($invitations); 
			if($invitations) {
				for( $i=0; $i<count($invitations); $i++ ){	
					...
					//To get proposal id
				echo	$proposal_sql = "SELECT id FROM #__cam_vendor_proposals WHERE rfpno = ".$rfps[$r]->id." and proposedvendorid=".$invitations[$i]->user_id." ";
					...
					$filename = $invitations[$i]->user_id."_Invitation.pdf";
					$pdf = new FPDF('P', 'cm', 'A4');
					$pdf->open();
					$pdf->AddPage();
					$pdf->MultiCell(8,1,$body,1,'L');
					//$pdf->Output('components/com_camassistant/compliance_reports/' . $filename, 'F');
