...
//					$txt = '<span class="'. $menuclass .'"'. $elementParameters .'>'. ($menuNode->name != '' ? $menuNode->name : '&nbsp;') .'</span>';
			...
			default:	// formerly case 2
				...
				//echo "<pre>"; print_r($menuNode); exit;
				$user = JFactory::getUser();
				
				if(isset($user->user_type) && $user->user_type == 11 && $menuNode->name == 'HOME')
				$menuNode->link = 'index.php?option=com_camassistant&controller=vendors&task=vendor_dashboard&Itemid=112';
				else if(isset($user->user_type) && $user->user_type == 12  && $menuNode->name == 'HOME') 
				$menuNode->link = 'index.php?option=com_camassistant&controller=rfpcenter&task=dashboard&Itemid=128';
				else if(isset($user->user_type) && $user->user_type == 13 && $menuNode->name == 'HOME')
				$menuNode->link = 'index.php?option=com_camassistant&controller=rfpcenter&task=dashboard&Itemid=125'; 
				else if(isset($user->user_type) && $user->user_type == 16 && $menuNode->name == 'HOME')
				$menuNode->link = 'index.php?option=com_camassistant&controller=rfpcenter&task=dashboard&Itemid=125'; 
				
				//else
				//$menuNode->link = 'index.php?option=com_content&view=frontpage&Itemid=1';
...
/* Start -- confirm dialog box for CreateRFP:  anil_25-08-2011 */				
				...
				$linkBegin	= '<a href="'. $menuNode->link .'" class="'. $menuclass .'"'. $elementParameters .' onclick="javascript:return mitem()" >'; } //inner
				} 
				else if( $cid == 'vendor_compliance_docs' ){
				$linkBegin	= '<a href="'. $menuNode->link .'" class="'. $menuclass .'"'. $elementParameters .' onclick="javascript:return mitem_c()" >';
				}
				else {
				$linkBegin	= '<a href="'. $menuNode->link .'" class="'. $menuclass .'"'. $elementParameters .' >';
				}
