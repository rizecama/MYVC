...
class ListExtendedMenuView extends AbstractExtendedMenuView {
	...
	function _renderMenuNodeList(&$menuNodeList, $level = 0, $hierarchy = array()) {
		...
		$result	= '';
		//$result	.= '<ul ';
		$result	.= '<ul class="dropdown dropdown-horizontal" ';
		...
		foreach(array_keys($menuNodeList) as $id) {
			...
			if(count($menuNodeList)-1==$parentcnt){
			$classext='noborder';
			}
			$result	.= '<li';
			if ($this->activeMenuClassContainer) {
				$result	.= ' class=" '.$this->getContainerMenuClassName($menuNode, $level).' '.$classext.'"';
			}
			if ($this->hierarchyBasedIds) {
			
			$count=count($this->getHierarchyString($itemHierarchy).$this->idSuffix);
			
		/*	  if($this->getHierarchyString($itemHierarchy)=='6'){
				$result	.= ' style="background:none;" id="menuitem_'.$this->getHierarchyString($itemHierarchy).$this->idSuffix.'"';
				} else {
