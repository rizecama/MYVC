...
$db =& JFactory::getDBO();
$sql_masters = "SELECT U.id,U.name,U.lastname,V.date,V.weeks FROM `jos_users` as U, `jos_cam_master_email_compliance_status` as V where U.user_type=13 and U.accounttype='master' and U.block=0 and U.id=V.masterid ";
...
echo "Reminder: ".$reminder.'<br />'; 
	...
	if( $reminder == '0' && $masters[$ms]->weeks > '0'){
	...
	$firms = '';
	$firmid1 = '';
	 for( $t=0; $t<count($total_managers); $t++ ){
	
		$query_preferred = "SELECT DISTINCT(U.v_id), V.company_name,V.company_phone, W.id, W.subscribe_type, W.subscribe from #__vendor_inviteinfo as U, #__cam_vendor_company as V, #__users as W where LOWER(U.inhousevendors) = (W.email) and W.block=0 and V.user_id=W.id and U.userid=".$total_managers[$t]->cust_id." and W.block=0 and W.search='' order by V.company_name ASC "; 
	...
	//Check the master have global standards or not
			$log = new checkstatus();
			$master	=	$log->getmasterfirmaccount($total_managers[$t]->cust_id);
			...
			if( $checkglobal == 'success' )	{
				...
				if($totalprefers_new_gli == 'success' && $totalprefers_new_aip == 'success' && $totalprefers_new_wci == 'success' && $totalprefers_new_umb == 'success' && $totalprefers_new_pln == 'success' && $totalprefers_new_occ == 'success' && $totalprefers_new_omi == 'success' )
				{
					...
					$terms = $db->loadResult();
						...
						$c_status_csv = 'Non-Compliant'.'red';
						}
					}
				}
				else
				...
				$vendor_documents_csv = $log->getexpired_documentscsv($preferred[$v]->id);
				...
				$vendor_data_csv[$vendor_industries[$in]->industry_name][] = $c_status_csv.'MYVC'.$account_type_csv .'MYVC' . $preferred[$v]->company_name .'MYVC'.$vendor_documents_csv.'MYVC' . $preferred[$v]->company_phone ;		
	...
	$final_per_mn = '';
				...
				if($total_managers[$t]->usertype == 'master'){
				...
				if($total_managers[$t]->usertype == 'admin'){
				...
				if($total_managers[$t]->usertype == 'dm'){
				...
				if($total_managers[$t]->usertype == 'm'){
	...
	//Completed 
		...
		$comp_stand = $table_start.$only_content.$table_end ;
		 ...
		 $j = 2;
		   ...
		   foreach($vendor_data_csv as $key=>$value){
      ...
      $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
		$title = $total_managers[$t]->cust_id.'_compliancereports.xlsx'; //set title
		$objWriter->save(str_replace(__FILE__, '/var/www/vhosts/vps61048.vps.ovh.ca/httpdocs/components/com_camassistant/assets/compliance_reports/'.$title ,__FILE__));
		        $otheremails = "SELECT email FROM #__cam_master_compliancereport_emails WHERE user_id=".$total_managers[$t]->cust_id."";
				$db->setQuery($otheremails);
				$otheremails = $db->loadObjectList();
				if($otheremails){
				   for( $e=0; $e<count($otheremails);$e++ )
				   {
				   $otheremail = $otheremails[$e]->email;
				   }
	           if( $docs_permission->documenttype == 0 )
			   {
				$message = "SELECT introtext FROM #__content WHERE  id=276";
				$db->setQuery($message);
				$body = $db->loadResult();
				$body = str_replace('{Manager}',$total_managers[$t]->name.' '.$total_managers[$t]->lastname,$body);
				...
				$sub = "Preferred Vendor Compliance Status_devcsv";
	...
	$attachment = "/var/www/vhosts/vps61048.vps.ovh.ca/httpdocs/components/com_camassistant/assets/compliance_reports/".$title;
	$to_manager = $total_managers[$t]->email ;
	...
	$successMail =JUtility::sendMail($from, $from_name, $otheremail, $sub, $body,$mode = 1, $cc=null, $bcc=null, $attachment, $replyto=null, $replytoname=null);
	...
	public function getexpired_documentscsv($vendorid){
		$gli_expdate = $this->gliexpdatecsv($vendorid);
		$aip_expdate = $this->aipexpdatecsv($vendorid);
		$wci_expdate = $this->wciexpdatecsv($vendorid);
		$umb_expdate = $this->umbexpdatecsv($vendorid);
		$omi_expdate = $this->omiexpdatecsv($vendorid);
		$oln_expdate = $this->olnexpdatecsv($vendorid);
		$pln_expdate = $this->plnexpdatecsv($vendorid);
		$wc_expdate = $this->wcexpdatecsv($vendorid);
	...
	public function gliexpdatecsv($vendorid){
		$db =& JFactory::getDBO();
		$today = date('Y-m-d');
	...
	public function aipexpdatecsv($vendorid){
		$db =& JFactory::getDBO();
		$today = date('Y-m-d');
		$user = JFactory::getUser();
	...
	public function wciexpdatecsv($vendorid){
		$db =& JFactory::getDBO();
		$today = date('Y-m-d');
		$user = JFactory::getUser();
	...
	public function umbexpdatecsv($vendorid){
		$db =& JFactory::getDBO();
		$today = date('Y-m-d');
		$user = JFactory::getUser();
	...
	public function omiexpdatecsv($vendorid){
		$db =& JFactory::getDBO();
		$today = date('Y-m-d');
		$user = JFactory::getUser();
	...
	public function olnexpdatecsv($vendorid){
		$db =& JFactory::getDBO();
		$today = date('Y-m-d');
		$user = JFactory::getUser();
	...
	public function plnexpdatecsv($vendorid){
		$db =& JFactory::getDBO();
		$today = date('Y-m-d');
		$user = JFactory::getUser();
	...
	public function wcexpdatecsv($vendorid){
		$db =& JFactory::getDBO();
		$today = date('Y-m-d');
		$user = JFactory::getUser();
