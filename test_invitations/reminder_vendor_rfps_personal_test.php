...
//print_r($personal_vendorss);exit;
for( $r=0; $r<count($rfps); $r++ ){
	...
	$previous_rfpdate = explode('-',$rfps[$r]->createdDate);
	$previous_rfpdate = $previous_rfpdate[2] . '-' . $previous_rfpdate[0] . '-' . $previous_rfpdate[1] ;
		...
		$invitations = $db->loadObjectList();
			...
			if($invitations) {
				for( $i=0; $i<count($invitations); $i++ ){
					...
					if( in_array($invitations[$i]->user_id,$personal_vendorss)  || $previous_rfpdate < date('Y-m-d') ) 
			           {	
