...
class JPagination extends JObject
{
	...
	function __construct($total, $limitstart, $limit)
	{
		...
		$displayedPages	= 5;
		$page_cnt=round($this->get('pages.total')/$displayedPages);
		
		$this->set( 'pages.start', (floor(($this->get('pages.current') -1) / $displayedPages)) * $displayedPages +1);
       if($page_cnt<$displayedPages && $this->get('pages.current')>4){
			$this->set( 'pages.start', $this->get('pages.current')-4);
		}
		if ($this->get('pages.start') + $displayedPages < $this->get('pages.total')) {
			$this->set( 'pages.stop', $this->get('pages.start') + $displayedPages);
		} else {
	...
	function getPagesCounter()
	{
		...
		//$html .= JText::sprintf('JPAGE_CURRENT_OF_TOTAL', $this->get('limitstart'), $this->get('limit'),'JPAGE_CURRENT_OF_TOTAL',$this->get('total'));
	...
	function getResultsCounter()
	{
		...
		if ($this->total > 0) {
			...
			$msg = 'Showing ' . $fromResult. ' - '. $toResult.' of '. $this->total;
			$html .= "\n".$msg;
