...
$attfilename[$j]->upload_doc = str_replace(' ','_',$attfilename[$j]->upload_doc);
...
//echo '<pre>'; print_r($attfilename[$j]);
	...
	//$attfilename[$j]->upload_doc; //.'<br>'; // Commented out by Xander (Jul5_2013), Creates 0x0A (\n) at start of file.
	//$dest_new.$docname;
  ...
  $sql = "SELECT proposedvendorid FROM #__cam_vendor_proposals  WHERE rfpno=".$rfp_id." AND  (proposaltype='Submit' or proposaltype='resubmit') AND Alt_bid != 'yes'";
		...
		$ZIPCOM = array();
		 ...
		 $dest_compilance = $newrfp_id.DS.$vendorname.DS.'VendorCompliance'.DS;
				...
				$ext = substr($OLN[$af], -3);
				$v1=$af+1;
           // echo  $path_compliance.'OLN_'.$v1.'/'.$OLN[$af].'<br>';
