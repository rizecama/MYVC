...
class UsersController extends JController
{
	...
	if ($user_type == '11')
	{
	        $query = "SELECT id FROM jos_users WHERE email='".$useremail."'";
			$db->setQuery($query);
			$result = $db->loadresult();
			$userid = $result; 
			 
			//to delete records from vendors table
			$query = 'DELETE FROM jos_cam_vendor_company WHERE user_id ='.$userid.' '; 
			$db->setQuery( $query );
			$db->query();
			
			//to delete records from users table
			 $query = 'DELETE FROM jos_users WHERE id ='.$userid.'';
			$db->setQuery( $query );
			$db->query();
			
			//to delete rocords from cam_rfpsow_addexception table
			$query = 'DELETE FROM jos_cam_rfpsow_addexception WHERE  vendor_id = '.$userid.'';
			$db->setQuery( $query );
			$db->query();
			//to delete records from 
			$query = 'DELETE FROM jos_cam_rfpsow_addnotes WHERE  vendor_id ='.$userid.''; 
			$db->setQuery( $query );
			$db->query();
			//to delete records from 
 			$query = 'DELETE FROM jos_cam_rfpsow_uploadfiles WHERE  vendor_id ='.$userid.''; 
			$db->setQuery( $query );
			$db->query();
			//to delete records from 
 			$query = 'DELETE FROM jos_cam_vendor_inviteinfo WHERE  v_id ='.$userid.''; 
			$db->setQuery( $query );
			$db->query();
	...
	$userid = $cids;
	   ...
	   $query1 = 'DELETE FROM jos_cam_customer_companyinfo WHERE cust_id ='.$userid.'';
			$db->setQuery( $query1 );
			$db->query();
		
			//to delete records from users table
			$query2 = 'DELETE FROM jos_users WHERE id = '.$userid.'';
			$db->setQuery( $query2 );
			$db->query();
			//to delete rocords from cam_rfpsow_addexception table
			 $query3 = 'DELETE FROM jos_cam_camfirminfo WHERE  cust_id ='.$userid.'';
			$db->setQuery( $query3 );
			$db->query();
			 $query4 = 'DELETE FROM jos_cam_invitemanagers WHERE  managerid = '.$userid.' ';
			$db->setQuery( $query4 );
			$db->query();
	}
	...
	{
	     $userid = $cids;
	       $query1 = 'DELETE FROM jos_cam_propertyowner_info WHERE user_id ='.$userid.'';
			$db->setQuery( $query1 );
			$db->query();
			...
			$db->query();
			//to delete rocords from cam_rfpsow_addexception table
			$query3 = 'DELETE FROM jos_cam_propertyowner_image WHERE  cust_id ='.$userid.'';
			$db->setQuery( $query3 );
			$db->query();
