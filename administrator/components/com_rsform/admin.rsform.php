...
function formsEdit($formId)
{
	...
	$db->setQuery("SELECT id,industry_name FROM #__cam_industries");
	$camidus = $db->loadobjectlist();
	$db->setQuery("SELECT * FROM #__rsform_forms WHERE FormId='".$formId."'");
	$row = $db->loadAssoc();

    rsform_HTML::formsEdit($formId, $row ,$camidus);
}
...
//    	`FormLayoutAutogenerate`= '{$row['FormLayoutAutogenerate']}',
	$db->setQuery("UPDATE #__rsform_forms SET
		...
		`camindustry` 				= '".$row['camindustry']."',
		`line_item_price` 		= '".$row['line_item_price']."',
