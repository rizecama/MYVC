...
// - - - - - - - - - - - - - - - 
// PDF
echo $pane->startPanel( JHTML::_( 'image.site', 'components/com_phocapdf/assets/images/icon-16-email.png','', '', '', '', '') . '&nbsp;'.JText::_('Email'), 'email' );
echo '<div style="font-size:1px;height:1px;margin:0px;padding:0px;">&nbsp;</div>';//because of IE bug

if($output = PhocaPDFHelperParams::renderMisc($this->params, 'params', 'phocaemail')) {
	echo $output;
} else {
	echo '<div style="text-align: center; padding: 5px;">'.JText::_('There are no parameters for this item').'</div>';
}
