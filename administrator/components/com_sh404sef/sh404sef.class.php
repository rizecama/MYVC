 ...
 * @version     $Id: sh404sef.class.php 2138 2011-11-14 18:39:59Z silianacom-svn $
 */
...
DEFINE ('sh404SEF_MAX_SEF_URL_LENGTH', 200);
...
function shComputeItemidString( $nonSefUrl, &$title, $shLangName) {
  ...
  if (!$shHomePageFlag) {  // we may have found that this is homepage, so we msut return an empty string
    ...
    if (!preg_match( '/Itemid=[0-9]+/i', $nonSefUrl)) { // if no Itemid in non-sef URL
      ...
      COM_SH404SEF_ALWAYS_INSERT_ITEMID_PREFIX.$sefConfig->replacement . shGetURLVar($nonSefUrl, 'Itemid')
      : '';
