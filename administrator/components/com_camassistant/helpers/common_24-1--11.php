...
if($_GET['type']=="states")
{
		for($i=0;$i<$_GET['fieldcnt'];$i++) 
		{
			...
			<?php if($i != 0){?>
			<div><?php /*?><a class="orange-links" href="javascript:void(0);" onclick="javascript: removeSpecificDiv('countydiv<?php echo $i;?>','fields_div_1','field_cnt<?php echo $i;?>');">remove</a><?php */?></div></div>
			<?php }else{?></div>
			<?php }
	...
	//$catList = mysql_fetch_array($result);
	  while($row = mysql_fetch_array($stateresult))
	  {
		...
		for($i=0;$i<$_GET['fieldcnt'];$i++) 
		{
			...
			</div><?php if($i != 0){?><div><?php /*?><a class="orange-links" href="javascript:void(0);" onclick="javascript: removeSpecificDiv('countydiv<?php echo $i;?>','fields_div_1','field_cnt<?php echo $i;?>');">remove</a><?php */?></div><?php }?>
			</div>
			</div>
			<a href="javascript:void(0);" onclick="javascript: getCounty(document.getElementById('stateid<?php echo $i;?>').value,'1','divmoreStates<?php echo $i;?>','field_cnt<?php echo $i;?>');" class="orange-links">Add Another County</a>
			<?php
		}
