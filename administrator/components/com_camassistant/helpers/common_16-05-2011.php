	...
	for($i=0;$i<$_GET['fieldcnt'];$i++) 
		{
			...
			?>
			 <div style="padding-top:10px;" id="countydiv<?php echo $i;?>">
             <?php if($i == 0){  ?>
             <div style="position:absolute; width:100px; margin-left:288px; ">
             <?php } else { ?>
             <div style="position:absolute; width:100px; margin-left:288px; margin-top:-12px;">
             <?php } ?>
             <img src="templates/camassistant_left/images/closecounty.gif" align="right"  onclick="closecounty('<?php echo $i;?>')" /></div><select name="county[]" <?PHP if($i==0) { ?> id="county_ID" <?PHP } ?>class="inputbox" style="width:282px"><option value="" >Please Select County</option>
		 	<?php
