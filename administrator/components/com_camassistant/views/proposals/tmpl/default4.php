...
var del_path = "&nbsp;<img onclick = \'javascript: warranty_doc_remove(); \' src=\'<?php echo Juri::root();; ?>templates/camassistant_left/images/delete.gif\' style=\" cursor:pointer;\" alt=\'Remove file\' />";
...
<!-- sof bedcrumb -->
<div style="float: right;"><a href="<?php echo $backlink;?>"><img alt="info" src="<? echo juri::root(); ?>templates/camassistant_left/images/back.png"></a></div>
<!-- eof bedcrumb -->
<!-- sof dotshead -->
...
<?php if($warranty=='file'){ ?>
	<a href="#" style="cursor:pointer" onclick="addfile('<?php echo $taskid; ?>','<?php echo $pro_path; ?>','<?php print_r($this->openfiles['files'][$i]); ?>');">Add To Proposal</a> <?php } else if($warranty=='sfile'){ ?>
...
<ul>
<li  style=" background:none;"><a class="modal" id="links1"  title="Add Folder"  href="index.php?option=com_camassistant&controller=documents&task=addfolder&parentid=1&pid=<?php echo $_REQUEST['pid']; ?>&mid=<?php echo $_REQUEST['mid']; ?>&path=<?php echo $_REQUEST['path']; ?>" rel="{handler: 'iframe', size: {x: 400, y: 200}}" ><img src="<? echo juri::root(); ?>images/new_folder.png" alt="add a new folder" width="45" height="39" align="absmiddle" /> </a><a class="modal" id="links1"  title="Add Folder"  href="index.php?option=com_camassistant&controller=documents&task=addfolder&parentid=1&pid=<?php echo $_REQUEST['pid']; ?>&mid=<?php echo $_REQUEST['mid']; ?>&path=<?php echo $_REQUEST['path']; ?>" rel="{handler: 'iframe', size: {x: 400, y: 200}}" >+Add a new Folder  </a></li>

<li><a class="modal" id="links1"  title="Upload a file"  href="index.php?option=com_camassistant&controller=documents&task=addfile&parentid=1&pid=<?php echo $_REQUEST['pid']; ?>&mid=<?php echo $_REQUEST['mid']; ?>&path=<?php echo $_REQUEST['path']; ?>" rel="{handler: 'iframe', size: {x: 500, y: 250}}" > +Upload a file to this folder <img src="<? echo juri::root(); ?>images/upload_folder.png" alt="Upload a file to this folder" width="37" height="36" align="middle" /></a></li>
</ul>
</div>
</div></div>
<div class="clear"></div>
