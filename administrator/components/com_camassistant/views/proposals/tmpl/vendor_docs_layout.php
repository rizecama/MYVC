...
var del_path = "&nbsp;<img onclick = \'javascript: warranty_doc_remove(); \' src=\'<? echo juri::root(); ?>templates/camassistant_left/images/delete.gif\'  style=\" cursor:pointer;\" alt=\'Remove file\' />";
...
window.parent.document.getElementById('delimg'+taskid).style.display ='';
//window.parent.document.getElementById('sfile_text'+taskid).innerHTML =filename;
...
<tr class="table_blue_rowdots2">
    <td align="center" valign="bottom"><img src="<? echo juri::root(); ?>templates/camassistant_inner/images/images_<?PHP echo $ext; ?>.png"  alt="folder" /></td>
...
<tr class="table_blue_rowdots2">
    <td align="center" valign="bottom"><img src="<? echo juri::root(); ?>images/folder_icon.png"  alt="folder" /></td>
...
$vendor_files->property_manager_id = $_REQUEST['vendorid'];
}
//$db =& JFactory::getDBO(); $user =&JFactory::getUser();
$vendor_files->property_manager_id = $_REQUEST['vendorid'];
?>
   <?php //echo '<pre>'; print_r($_REQUEST['vendorid']); ?>
<div class="table_bottomlinks2">
<ul>
<li  style=" background:none;"><a class="modal" id="links1"  title="Add Folder"  href="index.php?option=com_camassistant&controller=documents&task=addfolder&parentid=0&pid=<?php print_r($vendor_files->property_id); ?>&mid=<?php print_r($vendor_files->property_manager_id); ?>&path=<?php echo $val.$this->taxid[0]->tax_id; ?>" rel="{handler: 'iframe', size: {x: 400, y: 200}}" ><img src="images/new_folder.png" alt="add a new folder" width="45" height="39" align="absmiddle" /> </a><a class="modal" id="links1"  title="Add Folder"  href="index.php?option=com_camassistant&controller=documents&task=addfolder&parentid=0&pid=<?php print_r($vendor_files->property_id); ?>&mid=<?php print_r($vendor_files->property_manager_id); ?>&path=<?php echo $val.$this->taxid[0]->tax_id; ?>" rel="{handler: 'iframe', size: {x: 400, y: 200}}" >+Add a new Folder  </a></li>

<li><a class="modal" id="links1"  title="Upload File"  href="index.php?option=com_camassistant&controller=documents&task=addfile&parentid=0&pid=<?php print_r($vendor_files->property_id); ?>&mid=<?php print_r($vendor_files->property_manager_id); ?>&path=<?php echo $val.$this->taxid[0]->tax_id; ?>" rel="{handler: 'iframe', size: {x: 500, y: 250}}" > +Upload a file to this folder <img src="images/upload_folder.png" alt="Upload a file to this folder" width="37" height="36" align="middle" /></a></li>
</ul>
</div>
</div></div>
