...
error_reporting(0);
JToolBarHelper::title( JText::_( 'Old Invited Vendors' ));
...
el='<?php  echo Juri::base(); ?>index2.php?option=com_camassistant&controller=invitevendors&task=newinvitation';
var options = $merge(options || {}, Json.evaluate("{handler: 'iframe', size: {x: 750, y:500}}"))
SqueezeBox.fromElement(el,options);
}
else if(pressbutton == 'newemail'){
el='<?php  echo Juri::base(); ?>index2.php?option=com_camassistant&controller=invitevendors&task=newemailinvitation';
...
Vendor type: <select onchange="document.adminForm.submit();" style="width:200px;" size="1" id="vendortype" name="vendortype">
<option value="0">All Types</option>
<option <?php if($_REQUEST['vendortype'] == 'preferred') { ?> selected="selected"<?php }?> value="preferred">Preferred</option>
<option <?php if($_REQUEST['vendortype'] == 'inhouse') { ?> selected="selected"<?php }?> value="inhouse">In-House</option>
<option <?php if($_REQUEST['vendortype'] == 'exclude') { ?> selected="selected"<?php }?>value="exclude">Exclude</option>
</select>
...
<th class="title" align="center" width="150">
<?php echo JHTML::_( 'grid.sort', 'Vendor Type', 'vendortype', $this->lists['order_Dir'], $this->lists['order']); ?>
<?php  //echo sorting('Vendor Type', 'type'); ?>
</th>
...
<th class="title" align="center" width="200">
<?php echo JHTML::_( 'grid.sort', 'Invited Date', 'invitedate', $this->lists['order_Dir'], $this->lists['order']); ?>
</th>
...
<tr class="<?php echo "row$k"; ?>">
			...
			<td width="20">
				<input type="checkbox" onclick="isChecked(this.checked);" value="<?php echo $this->result[$i]->vid; ?>" name="cid[]" id="cb<?php echo $i; ?>">
			</td>
<td align="center" width="150"><?php echo $vendor->vendortype; ?></td>
<td align="center" width="200"><?php echo $vendor->inhousevendors; ?></td>
<td align="center" width="200"><?php echo $vendor->name."&nbsp;&nbsp;".$vendor->lastname; ?></td>
<td align="center" width="200"><?php echo $vendor->email; ?></td>

<?php
	$db = JFactory::getDBO();
	$vendor_id = "SELECT id from #__users  where email = '".$vendor->inhousevendors."' ";
...
<td align="center" width="200"><?php echo $vendor->invitedate; ?></td>
</tr>
