 ...
 * @since 1.5
 */
class customerModelcustomer extends JModel
{
	...
	{  
	  ...
	  //  $where1 = ' #__users ;
	   if($search){
		...
		// $query = 'SELECT u.user_type,u.id,u.lastname,u.name,u.email,u.registerDate,u.lastvisitDate,u.dmanager,u.phone,u.username,u.flag,u.suspend,u.password,u.block as published,c.camfirm_license_no,c.comp_id ,c.comp_name,c.tax_id,c.status,c.cust_id,u.accounttype FROM #__users as u LEFT JOIN #__cam_customer_companyinfo as c ON c.cust_id= u.id and u.user_type=13 '.$where.' group by u.id '.$orderby;
		if(!$search && $vendorlogs != 'log')	
		$usertpe = 'and u.user_type=13';
		else 
		$usertpe = '';
		
		$query = 'SELECT u.user_type,u.id,u.lastname,u.name,u.email,u.registerDate,u.lastvisitDate,u.dmanager,u.phone,u.username,u.flag,u.suspend,u.accounttype,u.password,u.block as published,c.camfirm_license_no,c.comp_id ,c.comp_name,c.tax_id,c.status,c.cust_id,u.accounttype  FROM #__users as u
		 LEFT JOIN #__cam_customer_companyinfo as c
		 ON c.cust_id= u.id and u.id not in(select firmid from #__cam_masteraccounts) '.$usertpe.' '.$where.' group by u.id '.$orderby;
