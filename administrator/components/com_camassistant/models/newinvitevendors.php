...
class newinvitevendorsModelnewinvitevendors extends Jmodel
{
	/**
	 * helloworld data
	 *
  ...
  /**
	...
	function _buildQuery()
	{
			...
			if($camid)
			$where[] = 'U.managerid="'.$camid.'"';
			$where[] = 'U.managerid=V.id';
			$wherecon=implode(' and ',$where);
			$query = "SELECT U.*,V.name,V.lastname,V.email FROM #__cam_newvendorinvitations as U, #__users as V where ".$wherecon."  ".$orderby.""; 
			//$query = "SELECT U.*,V.name,V.lastname,V.email FROM #__cam_newvendorinvitations as U, #__users as V where ".$wherecon_n." "; 
			return $query;
	 ...
	 function delete($id = array())
	{
		...
		if (count( $id ))
		{
			...
			$ids = implode( ',', $id );
		 	$query = 'DELETE FROM #__cam_newvendorinvitations where id IN ( '.$ids.' ) '; 
			$db->setQuery( $query );
			if(!$db->query()) {
...
//	echo "<pre>"; print_r($post); exit;
	...
	$db = JFactory::getDBO();
	$all_managers = "SELECT * from #__users  where user_type != 11 order by name ASC";
