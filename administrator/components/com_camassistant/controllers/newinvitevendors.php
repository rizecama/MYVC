...
class NewinvitevendorsController extends JController
{
	...
	function remove()
	{
		...
		$model = $this->getModel('newinvitevendors');
		if($res <= 0)
		{
			$model->delete($cid) ;
			/*if(!$model->delete($cid)) {
		...
		$this->setRedirect( 'index.php?option=com_camassistant&controller=newinvitevendors&camid='.$_REQUEST['camid'].'',$msg);
	 }
	 	function newexclude()
	{
		JRequest::setVar('view', 'newinvitevendors');
		parent::display();
	}
...
function inhousevendors(){
	...
	<?php } ?>
	window.parent.location.href = 'index.php?option=com_camassistant&controller=newinvitevendors';
   ...
   function deletevendor(){
		...
		$this->setRedirect( 'index.php?option=com_camassistant&controller=newinvitevendors',$msg);
