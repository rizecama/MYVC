...
class propertyownerController extends JController
{
...
/*    function unpublish()
	...
	function remove()
	{
		...
		$model = $this->getModel('propertyowner');
		if($res <= 0)
		{
			if(!$model->delete($cid)) {
		...
		$this->setRedirect( 'index.php?option=com_camassistant&controller=propertyowner',$msg);
	 }
	 function activation(){
	 ...
	 //echo "<pre>"; print_r($_REQUEST); exit;
		...
		$assignuseremail = 'sreenukuramdasu461@gmail.com';
		$success = JUtility::sendMail($mailfrom, $fromname, $assignuseremail, $mailsubject, $body,$mode = 1);
		//code to update status of vendor in users,vendor company tables 
		if($success){
		
		$query = 'UPDATE #__users'
		. ' SET block = "1", activation ="'.$vendor['activation'].'" WHERE id IN ( '. $user_id.'  )';
		$db->setQuery( $query );
		$db->query();
		$this->setRedirect( 'index.php?option=com_camassistant&controller=propertyowner','Email Verification Sending Successfully');	
				}
	 }
	 function block()
	{
