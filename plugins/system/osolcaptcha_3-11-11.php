...
class plgSystemOSOLCaptcha extends JPlugin
{
	   ...
	   function onAfterRoute()
        {
			...
			elseif(isset($_REQUEST['osolCatchaTxt']) && !$this->confirm($osolCatchaTxt,$osolCatchaTxtInst))
			{
				...
				//echo "<pre>"; print_r($camfirm_license_no); exit;
				$mainframe->redirect('index.php?option=com_camassistant&controller=propertymanager&camfirm_license_no='.$camfirm_license_no.'&view=propertymanager&Itemid=57');
				}
	...
	$mainframe->redirect('index.php?option=com_camassistant&controller=propertymanager&view=propertymanager&task=submit_licid&Itemid=105&tax_id='.$_REQUEST['tax_id'].'&taxid1='.$taxid1[0].'&taxid2='.$taxid1[1].'&Itemid='.$_REQUEST['Itemid']);
