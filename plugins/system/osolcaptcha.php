	   ...
	   function onAfterRoute()
        {
			...
			elseif(isset($_REQUEST['osolCatchaTxt']) && !$this->confirm($osolCatchaTxt,$osolCatchaTxtInst))
			{
				...
				JError::raiseWarning("666",'You have entered in an incorrect verification code.  Please scroll down to the bottom of the page and retype the proper letters.  If you need assistance, please call our office at 561-246-3830.');
				//JError::raiseWarning("666",JTEXT::_('OSOLCAPTCHA_ERROR_MESSAGE').JTEXT::_('OSOL_CAPTCHA_GO_BACK'));
				//JFactory::getApplication()
				//echo "<pre>"; print_r($_REQUEST); exit;
				//jimport('joomla.event.plugin');
				if($_REQUEST['task']=='submit_licid'){
				$camfirm_license_no= $_REQUEST['taxid1'].'-'.$_REQUEST['taxid2'];
					if($_REQUEST['taxid3']){
					$camfirm_license_no = $camfirm_license_no .'-'. $_REQUEST['taxid3'] ;
					}
				$from = 'invitation' ;
				$mainframe->redirect('index.php?option=com_camassistant&controller=propertymanager&camfirm_license_no='.$camfirm_license_no.'&from='.$from.'&view=propertymanager&Itemid=57');
				}
		if($_REQUEST['view']=='propertymanager'){
		//echo "<pre>"; print_r($_REQUEST); exit;
				$_SESSION['psignup']['tax_id']= $_REQUEST['tax_id']; 
				$_SESSION['psignup']['taxid2']= $_REQUEST['taxid2']; 
				$_SESSION['psignup']['taxid1']= $_REQUEST['taxid1']; 
				$_SESSION['psignup']['tax_id1']= $_REQUEST['tax_id1']; 
				$_SESSION['psignup']['comp_name']= $_REQUEST['comp_name']; 
				$_SESSION['psignup']['mailaddress']= $_REQUEST['mailaddress']; 
				$_SESSION['psignup']['city']= $_REQUEST['city']; 
				$_SESSION['psignup']['comp_state']= $_REQUEST['comp_state']; 
				$_SESSION['psignup']['zipcode']= $_REQUEST['zipcode']; 
				$_SESSION['psignup']['cphone1']= $_REQUEST['cphone1']; 
				$_SESSION['psignup']['cphone2']= $_REQUEST['cphone2']; 
				$_SESSION['psignup']['cphone3']= $_REQUEST['cphone3']; 
				$_SESSION['psignup']['cext']= $_REQUEST['cext']; 
				$_SESSION['psignup']['caphone1']= $_REQUEST['caphone1']; 
				$_SESSION['psignup']['caphone2']= $_REQUEST['caphone2']; 
				$_SESSION['psignup']['caphone3']= $_REQUEST['caphone3']; 
				$_SESSION['psignup']['caext']= $_REQUEST['caext']; 
				$_SESSION['psignup']['website']= $_REQUEST['website']; 
				$_SESSION['psignup']['image']= $_REQUEST['image']; 
				$_SESSION['psignup']['email']= $_REQUEST['email']; 
				$_SESSION['psignup']['user_email']= $_REQUEST['user_email']; 
				$_SESSION['psignup']['password']= $_REQUEST['password']; 
				$_SESSION['psignup']['password2']= $_REQUEST['password2']; 
				$_SESSION['psignup']['salutation']= $_REQUEST['salutation']; 
				$_SESSION['psignup']['fname']= $_REQUEST['fname']; 
				$_SESSION['psignup']['lname']= $_REQUEST['lname']; 
				$_SESSION['psignup']['phone1']= $_REQUEST['phone1']; 
				$_SESSION['psignup']['phone2']= $_REQUEST['phone2']; 
				$_SESSION['psignup']['phone3']= $_REQUEST['phone3']; 
				$_SESSION['psignup']['ext']= $_REQUEST['ext']; 
				$_SESSION['psignup']['question']= $_REQUEST['question']; 
				$_SESSION['psignup']['answer']= $_REQUEST['answer']; 
				$_SESSION['psignup']['hear']= $_REQUEST['hear']; 
				$_SESSION['psignup']['mphone1']= $_REQUEST['mphone1']; 
				$_SESSION['psignup']['mphone2']= $_REQUEST['mphone2']; 
				$_SESSION['psignup']['mphone3']= $_REQUEST['mphone3']; 
				$_SESSION['psignup']['terms']= $_REQUEST['terms']; 
				$_SESSION['psignup']['authorized_business']= $_REQUEST['authorized_business']; 
				
				?>	<input type="hidden" name="taxid1" value="<?php echo $_REQUEST['taxid1']; ?>" />
	<input type="hidden" name="taxid2" value="<?php echo $_REQUEST['taxid2']; ?>" />
	<input type="hidden" name="tax_id1" value="<?php echo $_REQUEST['tax_id1']; ?>" />
	<?php
	 $taxid = $_REQUEST['tax_id1'];
	  $taxid1 = explode('-',$taxid); 
	  if($taxid1[2]){
	$tax3 = "&taxid3=".$taxid1[2]."";
	}
	//print_r($_REQUEST); exit;
	$mainframe->redirect('index.php?option=com_camassistant&controller=propertymanager&view=propertymanager&task=submit_licid&Itemid=105&tax_id='.$_REQUEST['tax_id'].'&taxid1='.$taxid1[0].'&taxid2='.$taxid1[1].$tax3.'&Itemid='.$_REQUEST['Itemid']);
//$this->setLayout('company_info');
		//parent::display($tpl);
					//jimport('joomla.mail.helper');
              //$response->status = JAUTHENTICATE_STATUS_FAILURE;
//$response->error_message = 'Invalid username and password';
				//$mainframe->redirect('index.php?option=com_camassistant&controller=propertymanager&view=propertymanager&task=submit_companyinfo&Itemid=105&Itemid='.$_REQUEST['Itemid']);
		} else {
		//echo "<pre>"; print_r($_REQUEST); exit;
				...
				$mainframe->redirect('index.php?option=com_camassistant&controller=vendorsignup&task=vendorsignup_p2&Itemid='.$_REQUEST['Itemid']);
			}	
			}
			else
			{
  ...
  <tr>
	...
	".($vertical?"</tr><tr>":"")."
    <td align=\"left\" valign=\"middle\" style=\"padding-left:10px;\" ><input type=\"text\" name=\"osolCatchaTxt\" id=\"osolCatchaTxt{$GLOBALS['totalCaptchas']}\"  class=\"inputbox required validate-captcha\" />&nbsp;
	<input type=\"hidden\" name=\"osolCatchaTxtInst\" id=\"osolCatchaTxtInst\"  value=\"".$GLOBALS['totalCaptchas']."\"   /><br/>
	
	</td>
  </tr>
   <tr>
    <td valign=\"top\"  ".($vertical?"":"width=\"370\"")." >
	...
	function reloadCapthcha{$GLOBALS['totalCaptchas']}(instanceNo)
			...
			<label> 
       <a href=\"#\" onclick=\"reloadCapthcha{$GLOBALS['totalCaptchas']}(".$GLOBALS['totalCaptchas'].");return false;\" ><img src=\"templates/camassistant_left/images/refreshimg.jpg \" title=\" Letters Not Clear Click Here To New Captcha\"></a>
    </label></td>
   </tr>
  
</table>
</div>
");//.JText::_('REFRESH CAPTCHA')
		}
