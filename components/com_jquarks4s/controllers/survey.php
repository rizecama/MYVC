...
{    
    function display()
    {
        ...
        if ($accessId)
        {
		// anil 23-07-2011 start
		// To redirect the unauthorized users to login and if they login then they will redirected to survey directly
...
echo "<span class='blueheads'>You need to login to access this page</span>";
    ...
    function submitSurvey()
    {
        ...
        else
        {
            ...
            endforeach;
			/* anil 28-07-2011 Start //message to display after survey submission */
            //$msg = JText::_('THANKS_FOR_PARTICIPATION_SURVEY_SUBMITTED') ;
			$db = JFactory::getDBO();
			$sql = "UPDATE #__cam_rfpinfo SET survey='1' WHERE id='".$_REQUEST['rfpid']."'";
			$db->Setquery($sql);
			$db->query();
			$msg = 'Survey received - Thanks for your participation! Your feedback helps us determine the Vendor quality rating moving forward';
			/* anil 28-07-2011 End //message to display after survey submission */
        }
	...
	/* anil 28-07-2011 Start //message to display after survey submission and redirect to dashboard based on the user type login */
				//$url = 'index.php?option=com_jquarks4s';
				$user = JFactory::getUser();
				$db = & JFactory::getDBO();
				if($user->user_type == '11'){
					$url = 'index.php?option=com_camassistant&controller=vendors&task=vendor_dashboard&Itemid=112';
					}
					else if($user->user_type == '12'){
					$url = 'index.php?option=com_camassistant&controller=rfpcenter&task=dashboard&Itemid=128';
					}
	...
	/* anil 28-07-2011 End //message to display after survey submission and redirect to dashboard based on the user type login */
