...
class ContactController extends JController
{
	...
	function savedemo(){
		...
		$demo['firstname'] = JRequest::getVar('firstname','');
		$demo['lastname'] = JRequest::getVar('lastname','');
		$demo['companyname'] = JRequest::getVar('companyname','');
		$demo['phonenumber'] = JRequest::getVar('phonenumber','');
		$demo['email'] = JRequest::getVar('email','');
		$demo['rep'] = JRequest::getVar('rep','');
		$demo['pickdate'] = JRequest::getVar('pickdate','');
		$demo['spec'] = JRequest::getVar('spec','');
		$demo['demodate'] = $today = date("Y-m-d H:i:s"); 
		
		$phone_number = JRequest::getVar('phonenumber1','').'-'.JRequest::getVar('phonenumber2','').'-'.JRequest::getVar('phonenumber3','');
		
		$demo['copy'] = JRequest::getVar('copy','');
		
		$ssavedemo = $model->storedemo($demo);
		if($ssavedemo) {
			if( $demo['email'] ) {
				$sub="Need A Demo";
				$from_name = $demo['companyname']; 
				$from_email = $demo['email'];
				$support_to = 'support@myvendorcenter.com' ;
				//$support_to = 'simon.soa@gmail.com' ;
				$data = "SELECT introtext  FROM #__content where id='278'";
				$db->setQuery($data);
				$data = $db->loadResult();
				
				$date_selected = 'Can you please provide me a DEMO on <strong>'.$demo['pickdate'].'</strong>' ;
				$representative = 'Have a MyVC Representative contact me.' ;
				
				if($demo['pickdate'] == '') {
				$datatom = str_replace('[Representative]',$representative,$data );
				$datatom = str_replace('[For Date]','',$datatom );	
				}
				else{
				$datatom = str_replace('[For Date]',$date_selected,$data );
				$datatom = str_replace('[Representative]','',$datatom );
				}
				$datatom = str_replace('[companyname]',$demo['companyname'],$datatom );
				$datatom = str_replace('[username]',$demo['firstname'].'&nbsp;'.$demo['lastname'],$datatom  );			
				$datatom = str_replace('[Phone Number]',$phone_number,$datatom );			
				$body = $datatom;
				/*echo $from_email.'<br />' ;
				echo $from_name.'<br />' ;
				echo $support_to.'<br />' ;
				...
				echo $body.'<br />' ;
				exit;*/
				$successMail =JUtility::sendMail($from_email, $from_name, $support_to, $sub, $body,$mode = 1);
				if($demo['copy'] == 'on'){
				$to_user = $from_email ;
				$successMail =JUtility::sendMail($from_email, $from_name, $to_user, $sub, $body,$mode = 1);
				}
			}						
			$newURL = 'index.php';
