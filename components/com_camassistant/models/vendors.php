...
header("Content-Length: ".filesize($doc_name));*/
  ...
  /*  switch ($ext) {
	...
	function get_compliance_OMI_data()
	{
		$db = JFactory::getDBO();
		$user = JFactory::getUser();
...
//echo "<pre>"; print_r($data); print_r($row); exit;
	...
	//function to get OLN upload certificate link
	function get_OMI_upld_cert_link()
	{
	    /*****************code to send to fnd**************************/
	    JHTML::_('behavior.modal');
        $uri	=& JURI::getInstance();
...
//		$doc_name = $path."/".$filename;
	...
	function get_vendor_proposal_summary()
	{
		...
		$prp_Draft_sql = "SELECT VP.id, VP.Alt_bid, R.rfp_type,R.unawardeddate,VP.proposedvendorid,VP.proposaltype,R.awardeddate, VP.publish, R.id as rfp_id, R.projectName, R.proposalDueDate, R.proposalDueTime, P.id as Prperty_id, P.property_name,P.city, R.jobtype, P.divcounty  FROM  #__cam_vendor_proposals as VP
		...
		for($d=0,$c=0; $d<count($prp_Draft_summary); $d++)
		{
			...
			// To get the proposal count
			 $count = $this->gettotalbids($prp_Draft_summary[$d]->rfp_id);
			 $prp_Draft_summary[$d]->bids_count = $count;
		}
		...
		$today = date('m-d-Y');
		$prp_Submitted_sql = "SELECT VP.id, VP.Alt_bid, R.rfp_type,R.unawardeddate,VP.publish, R.awardeddate,VP.proposedvendorid, R.id as rfp_id, R.projectName, R.proposalDueDate, R.proposalDueTime,P.id as Prperty_id, P.property_name,P.city, R.jobtype, P.divcounty, VP.proposal_total_price  FROM  #__cam_vendor_proposals as VP
		LEFT JOIN #__cam_rfpinfo as R ON R.id = VP.rfpno
		LEFT JOIN #__cam_property as P ON R.property_id = P.id
		WHERE VP.proposedvendorid=".$user->id." AND (VP.proposaltype = 'Submit' OR VP.proposaltype = 'resubmit') AND STR_TO_DATE(R.proposalDueDate, '%m-%d-%Y') >= '".$today."' order by R.proposalDueDate ";
		//and R.rfp_type != 'closed' and R.close_vendor != 'closed';
		...
		for($S=0,$R=0; $S<count($prp_Submitted_summary); $S++)
		{
			 ...
			 /* $R_prp_Draft_summary[$R] = $prp_Submitted_summary[$S];
			  ...
			  // To get the proposal count
			 $count = $this->gettotalbids($prp_Submitted_summary[$S]->rfp_id);
			 $prp_Submitted_summary[$S]->bids_count = $count;
		}
		...
		$prp_Review_sql = "SELECT VP.id,VP.Alt_bid, VP.proposedvendorid,R.unawardeddate, R.awardeddate,R.rfp_type,R.id as rfp_id, R.projectName, R.proposalDueDate, R.proposalDueTime, P.id as Prperty_id, P.property_name, P.city, R.jobtype  FROM  #__cam_vendor_proposals as VP
		LEFT JOIN #__cam_rfpinfo as R ON R.id = VP.rfpno
		LEFT JOIN #__cam_property as P ON R.property_id = P.id
	...
	function getuserinfo()
	{
	    $db = JFactory::getDBO();
		$user = JFactory::getUser();
		$sql = "SELECT U.*, V.id as vendor_id, V.image, V.company_phone,V.alt_phone,V.phone_ext,V.alt_phone_ext, V.company_address,V.company_addresss, V.zipcode, V.faxid, V.fax_acc FROM #__users as U LEFT JOIN #__cam_vendor_company as V ON U.id = V.user_id WHERE U.id = ".$user->id;
		$db->Setquery($sql);
...
//echo "<pre>"; print_r($All_prp_Draft_summary); exit;
	...
	function _buildQuery($Prp_type)
	{
		...
		$condition = " AND (VP.proposaltype = '".$Prp_type."' )";
		 $query = "SELECT VP.id,VP.Alt_bid,VP.proposedvendorid,VP.proposaltype, R.close_vendor, R.rfp_type, VP.publish, VP.proposal_total_price ,R.id as rfp_id, R.projectName, R.proposalDueDate, R.proposalDueTime, P.id as Prperty_id, P.property_name, P.city, P.copyvendor, R.jobtype, R.awardeddate, P.divcounty  FROM  #__cam_vendor_proposals as VP
		LEFT JOIN #__cam_rfpinfo as R ON R.id = VP.rfpno
		LEFT JOIN #__cam_property as P ON R.property_id = P.id
		WHERE VP.proposedvendorid=".$user->id.$condition."  and R.awardjobaproval !=1 and R.awardjobdecline !=2 order by STR_TO_DATE(R.proposalDueDate, '%m-%d-%Y') DESC ";
		//AND ".$due_date."
       ...
       // $records = $this->_getList($query);
		for($d=0,$c=0; $d<count($records); $d++)
		{
			...
			$time_left_details = $this->fnCalculateEndDateTime($duedate);
			 ...
			 $records[$d]->access = 'No';
			 // To get the proposal count
			 $count = $this->gettotalbids($records[$d]->rfp_id);
			 $records[$d]->bids_count = $count;
...
//		echo ' states <pre>'; print_r($states);exit;
	   ...
	   function get_vendor_proposal_summary_dashboard()
	{
		...
		//query to get submitted proposals
		$today = date('m-d-Y');
		$prp_Submitted_sql = "SELECT VP.id, VP.Alt_bid, R.close_vendor,VP.publish, P.divcounty, P.city,VP.proposedvendorid, R.id as rfp_id, R.projectName, R.proposalDueDate, R.rfp_type, R.proposalDueTime,P.id as Prperty_id, P.property_name, R.jobtype, VP.proposal_total_price  FROM  #__cam_vendor_proposals as VP
		LEFT JOIN #__cam_rfpinfo as R ON R.id = VP.rfpno
		LEFT JOIN #__cam_property as P ON R.property_id = P.id
  ...
  $query = "SELECT DISTINCT(A.rfp_id), A.id, A.prp_id,R.projectName,R.proposalDueDate,A.not_interested,P.property_name,P.property_name,P.divcounty,R.property_id,R.industry_id,R.proposalDueTime,A.status,R.jobtype,R.create_rfptype  FROM jos_cam_vendor_availablejobs as A
				  ...
				  WHERE I.rfpid = A.rfp_id AND I.vendorid = A.user_id AND A.status = 0 AND R.rfp_type = 'rfp' AND  A.user_id=".$user->id." AND R.publish=1 and A.publish=0 and R.rfpapproval !=1 and R.rfpapproval_decline !=2 order by R.proposalDueDate,R.proposalDueTime  ASC " ;
		...
		$sql_city = "SELECT city, property_name FROM #__cam_property WHERE id = ".$res_records[$t]['property_id']." ";
		$db->Setquery($sql_city);
		$xyz_city = $db->loadObject();
		$res_records[$t]['City'] = $xyz_city->city;
		$res_records[$t]['Propertyname'] = $xyz_city->property_name;
...
//			echo "<pre>"; print_r($time_left_details);
	...
	function get_compliance_OMI_datav($user)
	{
		$db = JFactory::getDBO();
		$sql = "SELECT * FROM #__cam_vendor_errors_omissions_insurance WHERE vendor_id=".$user;
		$db->Setquery($sql);
		$OMI_data = $db->loadObjectList();
		return $OMI_data;
	}
	...
	$user = JFactory::getUser();
	$db = & JFactory::getDBO();
	$getvendordata = "SELECT U.id, U.name, U.email, U.lastname, V.company_phone, V.company_name, V.company_address, V.company_addresss, U.cellphone, V.phone_ext, V.image, V.established_year, V.zipcode, V.city, V.state, V.company_web_url, U.subscribe_type, V.alt_phone FROM #__users as U, #__cam_vendor_company  as V where  U.id=V.user_id and U.id=".$userid." ";
...
//Function to get userid
	...
	function getpreferredclist(){
		...
		and V.inhousevendors ='".$user->email."' and W.accounttype='master' group by U.comp_name having count(*)>=1";
		$db->setQuery($query);
        $camfirmslist = $db->loadObjectList();
		echo "<pre>"; print_r($camfirmslist); echo "</pre>";*/
...
function getomilibilities(){
		...
		if($omidata)
		{
			$g_data = '<table width="100%" border="0" style="font-size:24px;"><tr><td><strong>ERRORS & OMISSIONS INSURANCE</strong></td></tr><tr><td><ul style="list-style-type:none;">';
		}
...
//Completed
	...
	function gettotalbids($rfpid){
		$db =& JFactory::getDBO();
		$user = JFactory::getUser();
		$vendor_proposal = "SELECT count(id)  FROM #__cam_vendor_proposals where rfpno=".$rfpid." and proposedvendorid=".$user->id." ";
		$db->setQuery($vendor_proposal);
		$count_bids = $db->loadResult();
		return $count_bids;
	}
	
	//Function to remove data from proposal table
	function delete_proposals($rfpid,$pid,$altp){
		$db =& JFactory::getDBO();
		$user = JFactory::getUser();
		$query_del_proposal = "DELETE FROM #__cam_vendor_proposals WHERE proposedvendorid='".$user->id."' and rfpno='".$rfpid."' and id='".$pid."' and Alt_bid='".$altp."' ";
		$db->setQuery($query_del_proposal); 
		$dleete = $db->query() ;
		return $dleete ;
	}
	
	//Function to remove data from proposal table
	function delete_uploadfiles($rfpid,$pid,$altp){
		$db =& JFactory::getDBO();
		$user = JFactory::getUser();
		$query_uploadfiles = "DELETE FROM #__cam_rfpsow_uploadfiles WHERE vendor_id='".$user->id."' and rfp_id='".$rfpid."' and Alt_bid='".$altp."' "; 
		$db->setQuery($query_uploadfiles); 
		$dleete = $db->query() ;
		return $dleete ;
	}
	
	//Function to remove data from proposal table
	function delete_lineitems_prices($rfpid,$pid,$altp){
		$db =& JFactory::getDBO();
		$user = JFactory::getUser();
		$query_prices = "DELETE FROM #__cam_rfpsow_docs_lineitems_prices WHERE vendor_id='".$user->id."' and rfp_id='".$rfpid."' and Alt_bid='".$altp."' "; 		$db->setQuery($query_prices); 
		$dleete = $db->query() ;
		return $dleete ;
	}
	
	//Function to remove data from proposal table
	function delete_addnotes($rfpid,$pid,$altp){
		$db =& JFactory::getDBO();
		$user = JFactory::getUser();
		$query_addnotes = "DELETE FROM #__cam_rfpsow_addnotes WHERE vendor_id='".$user->id."' and rfp_id='".$rfpid."' and Alt_bid='".$altp."' "; 
		$db->setQuery($query_addnotes); 
		$dleete = $db->query() ;
		return $dleete ;
	}
	
	//Function to remove data from proposal table
	function delete_addexception($rfpid,$pid,$altp){
		$db =& JFactory::getDBO();
		$user = JFactory::getUser();
		$query_addexception = "DELETE FROM #__cam_rfpsow_addexception WHERE vendor_id='".$user->id."' and rfp_id='".$rfpid."' and Alt_bid='".$altp."' "; 
		$db->setQuery($query_addexception); 
		$dleete = $db->query() ;
		return $dleete ;
	}
	
	//Function to remove data from proposal table
	function delete_generalfiles($rfpid,$pid,$altp){
		$db =& JFactory::getDBO();
		$user = JFactory::getUser();
		$query_generalfiles = "DELETE FROM #__cam_generalfiles WHERE vendorid='".$user->id."' and rfpid='".$rfpid."' and proposalid='".$pid."' "; 
		$db->setQuery($query_generalfiles); 
		$dleete = $db->query() ;
		return $dleete ;
	}
	...
	//Function to remove data from proposal table
	function update_proposals($rfpid,$pid,$altp){
		$db =& JFactory::getDBO();
		$user = JFactory::getUser();
		$query_update_proposal = "UPDATE #__cam_vendor_proposals SET Alt_bid='' WHERE proposedvendorid='".$user->id."' and rfpno='".$rfpid."' and Alt_bid='yes' "; 
		$db->setQuery($query_update_proposal); 
		$updates = $db->query() ;
		return $updates ;
	}
	
	//Function to remove data from proposal table
	function update_uploadfiles($rfpid,$pid,$altp){
		$db =& JFactory::getDBO();
		$user = JFactory::getUser();
		$query_update_uploadfiles = "UPDATE #__cam_rfpsow_uploadfiles SET Alt_bid='' WHERE vendor_id='".$user->id."' and rfp_id='".$rfpid."' and Alt_bid='yes'";
		$db->setQuery($query_update_uploadfiles); 
		$updates = $db->query() ;
		return $updates ;
	}
	
	//Function to remove data from proposal table
	function update_lineitems_prices($rfpid,$pid,$altp){
		$db =& JFactory::getDBO();
		$user = JFactory::getUser();
		$query_update_prices = "UPDATE #__cam_rfpsow_docs_lineitems_prices SET Alt_bid='' WHERE vendor_id='".$user->id."' and rfp_id='".$rfpid."' and Alt_bid='yes' "; 		
		$db->setQuery($query_update_prices); 
		$updates = $db->query() ;
		return $updates ;
	}
	
	//Function to remove data from proposal table
	function update_addnotes($rfpid,$pid,$altp){
		$db =& JFactory::getDBO();
		$user = JFactory::getUser();
		$query_updates_addnotes = "UPDATE #__cam_rfpsow_addnotes SET Alt_bid='' WHERE vendor_id='".$user->id."' and rfp_id='".$rfpid."' and Alt_bid='yes' "; 
		$db->setQuery($query_updates_addnotes); 
		$updates = $db->query() ;
		return $updates ;
	}
	
	//Function to remove data from proposal table
	function update_addexception($rfpid,$pid,$altp){
		$db =& JFactory::getDBO();
		$user = JFactory::getUser();
		$query_update_addexception = "UPDATE #__cam_rfpsow_addexception SET Alt_bid='' WHERE vendor_id='".$user->id."' and rfp_id='".$rfpid."' and Alt_bid='yes' "; 
		$db->setQuery($query_update_addexception); 
		$updates = $db->query() ;
		return $updates ;
	}
	
	//Function to remove data from proposal table
	function update_generalfiles($rfpid,$pid,$altp){
		$db =& JFactory::getDBO();
		$user = JFactory::getUser();
		$query_update_generalfiles = "UPDATE #__cam_generalfiles SET proposal_type='' WHERE vendorid='".$user->id."' and rfpid='".$rfpid."' and proposal_type='yes' "; 
		$db->setQuery($query_update_generalfiles); 
		$updates = $db->query() ;
		return $updates ;
	}
	
	//Function to get jobtype and job id
	function getproposalinfo($rfpid){
		$db =& JFactory::getDBO();
		$user = JFactory::getUser();
		$proposalinfo = "SELECT A.id, V.jobtype, V.cust_id, V.property_id, V.rfp_type, V.create_rfptype  FROM #__cam_vendor_availablejobs as A, #__cam_rfpinfo as V where V.id=".$rfpid." and A.rfp_id=".$rfpid." and A.user_id=".$user->id." and A.rfp_id=V.id ";
		$db->setQuery($proposalinfo);
		$ancdata = $db->loadObject();
		return $ancdata;
	}
	//Function to get manager information
	function getmanagerinfo($rfpid, $propertyid, $customerid){
		$db =& JFactory::getDBO();
		$user = JFactory::getUser();
		
		$managerinfo = "SELECT A.name, A.lastname, A.email, A.cellphone, C.comp_name, C.comp_zip, C.comp_city, C.comp_phno, C.comp_extnno, A.showphone, A.showfax, A.showaddress, A.showcompany, A.showemail FROM #__users as A, #__cam_customer_companyinfo as C where C.cust_id=".$customerid." and A.id=C.cust_id ";
		$db->setQuery($managerinfo);
		$mgrinfo = $db->loadObject();
		return $mgrinfo;
	}
	//Function to get propertyowner information
	function getpropertyownerinfo($rfpid, $propertyid, $customerid){
		$db =& JFactory::getDBO();
		$user = JFactory::getUser();
	$propertyownerinfo = "SELECT A.name, A.lastname, A.email, A.cellphone,A.phone,A.user_type,A.extension FROM #__users as A, #__cam_propertyowner_info as C where C.user_id=".$customerid." and A.id=C.user_id ";
		$db->setQuery($propertyownerinfo);
		$mgrinfo = $db->loadObject();
	...
	function getpropsinfo($propertyid){
		$db =& JFactory::getDBO();
		$propsinfo = "SELECT street, city, state, divcounty, zip FROM #__cam_property where id=".$propertyid." ";
		$db->setQuery($propsinfo);
		$prpinfo = $db->loadObject();
		return $prpinfo;
	}
	//Function to get proposal is uninvited or not
	function proposaluninvited($rfpid){
		$db =& JFactory::getDBO();
		$user = JFactory::getUser();
		$sql_un = "SELECT proposaltype FROM #__cam_vendor_proposals WHERE rfpno=".$rfpid." and proposedvendorid = ".$user->id." ";
		$db->Setquery($sql_un);
		$uninvited = $db->loadResult();
		return $uninvited;
	}
	//Function to get draft proposal is uninvited or not
	function proposaluninvited_drafts($rfpid,$proposalid){
		$db =& JFactory::getDBO();
		$user = JFactory::getUser();
		$sql_un = "SELECT proposaltype, Alt_bid, publish FROM #__cam_vendor_proposals WHERE rfpno=".$rfpid." and proposedvendorid = ".$user->id." and id=".$proposalid." ";
		$db->Setquery($sql_un);
		$uninvited = $db->loadObject();
		return $uninvited;
	}
	//Function to get the count of links to create alternate proposal link
	function gettotalbids_newlink($rfpid){
	$db =& JFactory::getDBO();
	$user = JFactory::getUser();
	$sql = "SELECT count(rfpno) FROM #__cam_vendor_proposals where rfpno=".$rfpid." and proposedvendorid = ".$user->id."  group by rfpno";
	$db->Setquery($sql);
	$count_bids = $db->loadResult();
	return $count_bids;
	}		
	//Function to get all exopired compliance documents list
	function expireddocuments(){
		$gli_count = $this->glicount();
		$aip_count = $this->aipcount();
		$wci_count = $this->wcicount();
		$umb_count = $this->umbcount();
		$omi_count = $this->omicount();
		$oln_count = $this->olncount();
		$pln_count = $this->plncount();
		$wc_count = $this->wccount();
		$expired_docs = $gli_count + $aip_count + $wci_count + $umb_count + $omi_count + $oln_count + $pln_count + $wc_count ;
	...
	function glicount(){
		...
		$today = date('Y-m-d');
		$user = JFactory::getUser();
		$sql = "SELECT count(GLI_end_date) FROM #__cam_vendor_liability_insurence  where GLI_end_date <'".$today."' and GLI_end_date!='0000-00-00' and vendor_id=".$user->id;
		$db->Setquery($sql);
		$GLI_count = $db->loadResult();
	...
	function aipcount(){
		...
		$today = date('Y-m-d');
		$user = JFactory::getUser();
		$sql = "SELECT count(aip_end_date) FROM #__cam_vendor_auto_insurance where aip_end_date <'".$today."' and aip_end_date!='0000-00-00'  and vendor_id=".$user->id;
		$db->Setquery($sql);
		$AIP_count = $db->loadResult();
		return $AIP_count;
	}
	function wcicount(){
		$db =& JFactory::getDBO();
		$today = date('Y-m-d');
		$user = JFactory::getUser();
		$sql = "SELECT count(WCI_end_date) FROM #__cam_vendor_workers_companies_insurance where WCI_end_date <'".$today."' and WCI_end_date!='0000-00-00' and vendor_id=".$user->id;
		$db->Setquery($sql);
		$WCI_count = $db->loadResult();
		return $WCI_count;
	}
	function umbcount(){
		$db =& JFactory::getDBO();
		$today = date('Y-m-d');
		$user = JFactory::getUser();
		$sql = "SELECT count(UMB_expdate) FROM #__cam_vendor_umbrella_license where UMB_expdate <'".$today."' and UMB_expdate!='0000-00-00' and vendor_id=".$user->id;
		$db->Setquery($sql);
		$UMB_count = $db->loadResult();
		return $UMB_count;
	}
	function omicount(){
		$db =& JFactory::getDBO();
		$today = date('Y-m-d');
		$user = JFactory::getUser();
		$sql = "SELECT count(OMI_end_date) FROM #__cam_vendor_errors_omissions_insurance where OMI_end_date <'".$today."' and OMI_end_date!='0000-00-00' and vendor_id=".$user->id;
		$db->Setquery($sql);
		$OMI_count = $db->loadResult();
	...
	function olncount(){
		...
		$today = date('Y-m-d');
		$user = JFactory::getUser();
		$sql = "SELECT count(OLN_expdate) FROM #__cam_vendor_occupational_license where OLN_expdate <'".$today."' and OLN_expdate!='0000-00-00' and vendor_id=".$user->id;
		$db->Setquery($sql);
		$OLN_count = $db->loadResult();
	...
	function plncount(){
		...
		$today = date('Y-m-d');
		$user = JFactory::getUser();
		$sql = "SELECT count(PLN_expdate) FROM #__cam_vendor_professional_license where PLN_expdate <'".$today."' and PLN_expdate!='0000-00-00' and vendor_id=".$user->id;
		$db->Setquery($sql);
		$PLN_count = $db->loadResult();
		return $PLN_count;
	}
	function wccount(){
		$db =& JFactory::getDBO();
		$today = date('Y-m-d');
		$user = JFactory::getUser();
		$sql = "SELECT count(wc_end_date) FROM #__cam_vendor_workers_compansation where wc_end_date <'".$today."' and wc_end_date!='0000-00-00' and vendor_id=".$user->id;
		$db->Setquery($sql);
		$WC_count = $db->loadResult();

		return $WC_count;
	}
	
	function getnoncompcount(){
        $property = array(); 
		$db =& JFactory::getDBO();
		$user =& JFactory::getUser();
        $user_id = $user->get('email');
		$query = "SELECT V.userid, V.fei, V.invitedate, V.vendortype, V.status, U.comp_state, U.comp_name, U.comp_city, W.name,W.lastname from #__vendor_inviteinfo as V, #__cam_customer_companyinfo  as U, #__users as W where W.id=V.userid and V.userid =U.cust_id and V.vendortype !='exclude' and V.inhousevendors ='".$user_id."'  ";
		$db->setQuery($query);
		...
		for( $p=0; $p<count($pending); $p++ ){
			...
			$master =	$this->checkcompliancestatus_count($pending[$p]->userid);
						...
						foreach($only_state as $statues){
							$final_state[] = $statues->status;
								if(in_array('fail',$final_state)){
									...
									$masteraccount = $this->getmasterfirmaccount_count($pending[$s]->userid);
									$sql_terms = "SELECT termsconditions FROM #__cam_vendor_aboutus WHERE vendorid=".$masteraccount." "; 
									$db->setQuery($sql_terms);
									$terms_exist = $db->loadResult();
									if($terms_exist == '1'){
		...
		for( $t=0; $t<count($pending); $t++ ){
				$master =	$this->getmasterfirmaccount_count($pending[$t]->userid);
		...
		for ($i=0; $i<count($pending); $i++){
			if( $pending[$i]->final_status == 'fail' ) 
			$noc_count++ ;
		}
		return $noc_count;
	}
	
	function checkcompliancestatus_count($managerid){
		$db=&JFactory::getDBO();
		$user =& JFactory::getUser();
		$master =	$this->getmasterfirmaccount_count($managerid);
		$vendorindustrieslist = $this->vendorallindustries($user->id);
			for( $vi=0; $vi<count($vendorindustrieslist); $vi++ )
				{
					$totalprefers_new_w9	=	$this->checknewspecialrequirements_w9($user->id,$vendorindustrieslist[$vi]->value,$master);
					$totalprefers_new_gli	=	$this->checknewspecialrequirements_gli_indus($user->id,$vendorindustrieslist[$vi]->value,$master);
					$totalprefers_new_aip	=	$this->checknewspecialrequirements_aip_indus($user->id,$vendorindustrieslist[$vi]->value,$master);
					$totalprefers_new_wci	=	$this->checknewspecialrequirements_wci_indus($user->id,$vendorindustrieslist[$vi]->value,$master);
					$totalprefers_new_umb	=	$this->checknewspecialrequirements_umb_indus($user->id,$vendorindustrieslist[$vi]->value,$master);
					$totalprefers_new_pln	=	$this->checknewspecialrequirements_pln_indus($user->id,$vendorindustrieslist[$vi]->value,$master);
					$totalprefers_new_occ	=	$this->checknewspecialrequirements_occ_indus($user->id,$vendorindustrieslist[$vi]->value,$master);
					
					/*if($master == '1885'){
					echo "W".$totalprefers_new_w9 ;
...
if($totalprefers_new_w9 == 'fail' || $totalprefers_new_gli == 'fail' || $totalprefers_new_aip == 'fail' || $totalprefers_new_wci == 'fail' || $totalprefers_new_umb == 'fail' || $totalprefers_new_pln == 'fail' || $totalprefers_new_occ == 'fail' ){
	...
	function checknewspecialrequirements_w9($vendorid,$industryid,$managerid){
		...
		$occur_w9 = '';
	 	...
	 	function getmasterfirmaccount_count($manager){
	$user =& JFactory::getUser();
		$db=&JFactory::getDBO();
	...
	//Function to get JOB invitation into PDF file
	function getmessagepdf($rfpid, $proposalid){
		$vendor_companyname = $this->getvendor_companyname($proposalid);
		$manager_name = $this->getmanagername($rfpid);
		$propertyname = $this->getpropertyname($rfpid);
		$body = $this->getarticlemessage();
		
		$body = str_replace('[Vendor Company Name]',$vendor_companyname,$body);
		$body = str_replace('[Manager Name]',$manager_name->name.' '.$manager_name->lastname,$body);
		$body = str_replace('[Property Name]',str_replace('_',' ',$propertyname),$body);
		//$rfps[$r]->projectName
		$body = str_replace('[RFP Name]',$manager_name->projectName,$body);
		$rfpid = '('.$rfpid.')';
		$body = str_replace('(RFP #)',$rfpid,$body);
		return $body;
	}
	
	function getvendor_companyname($proposalid){
		$db=&JFactory::getDBO();
		$sql_vendor_cname = "SELECT U.company_name FROM #__cam_vendor_company as U, #__cam_vendor_proposals as V WHERE V.id=".$proposalid." and V.proposedvendorid = U.user_id"; 
		$db->setQuery($sql_vendor_cname);
		$vendorcname = $db->loadResult();
		return $vendorcname;
	} 
	
	function getmanagername($rfpid){
		$db=&JFactory::getDBO();
		$sql_managername = "SELECT U.name, U.lastname, V.projectName FROM #__users as U, #__cam_rfpinfo as V WHERE V.id=".$rfpid." and V.cust_id = U.id"; 
		$db->setQuery($sql_managername);
		$managername = $db->loadObject();
		return $managername;
	} 
	
	function getpropertyname($rfpid){
		$db=&JFactory::getDBO();
		$sql_propertyname = "SELECT U.property_name FROM #__cam_property as U, #__cam_rfpinfo as V WHERE V.id=".$rfpid." and V.property_id = U.id"; 
	...
	function get_open_invitations(){
       ...
       $date_1 = date('Y-m-d') ;
		...
		//echo "<pre>"; print_r($res_records); exit;
		//code to get county, industry name
		for($t=0; $t<count($res_records); $t++)
		{
		//echo $res_records[$t]['divcounty']; exit;
