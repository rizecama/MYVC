...
var $_pagination = null;
	...
	$totalprefers_new_pln	=	$this->checknewspecialrequirements_pln($awardjob[$sp]->proposedvendorid,$rfpid);
		...
		if($totalprefers_new_w9 == 'success' && $totalprefers_new_gli == 'success' && $totalprefers_new_aip == 'success' && $totalprefers_new_wci == 'success' && $totalprefers_new_umb == 'success' && $totalprefers_new_pln == 'success'){
...
$query = "SELECT U.id,U.projectName,U.proposalDueDate,U.proposalDueTime,V.property_name,U.cust_id,U.publish,V.timezone,U.jobtype FROM #__cam_rfpinfo as U LEFT JOIN #__cam_property as V ON U.property_id = V.id WHERE U.rfp_type = 'rfp' and U.publish!='2' and (U.cust_id='".$user_id."' ".$condition.") ORDER BY U.secondstime ASC";
		}
		}
		else{
		$query = "SELECT U.id,U.projectName,U.proposalDueDate,U.proposalDueTime,V.property_name,U.publish,V.timezone,U.jobtype FROM #__cam_rfpinfo as U LEFT JOIN #__cam_property as V ON U.property_id = V.id WHERE U.rfp_type = 'rfp' and U.publish!='2' and U.cust_id='".$user_id."' ORDER BY U.secondstime ASC";
		}
//echo $query; exit;
...
$query = "SELECT U.id,U.projectName,U.proposalDueDate,U.proposalDueTime,V.property_name,U.cust_id,U.publish,V.timezone,U.jobtype FROM #__cam_rfpinfo as U LEFT JOIN #__cam_property as V ON U.property_id = V.id WHERE U.rfp_type = 'rfp' and U.publish!='2' and ".$condition." ORDER BY U.secondstime ASC";
...
$query= "SELECT U.id,U.projectName,U.createdDate,U.proposalDueDate,U.proposalDueTime,V.property_name,U.publish,U.rfp_type,V.timezone,U.jobtype FROM #__cam_rfpinfo as U LEFT JOIN #__cam_property  as V ON U.property_id = V.id WHERE (U.rfp_type = 'rfp' and U.publish != 2) and ".$condition." ORDER BY U.createdDate DESC";
...
// To get the query for master account
	...
	$db->setQuery($query_mans);
		...
		$totalcust_id1_arr = implode($total_mangrs,',');
		$condition = " U.cust_id IN (".$totalcust_id1_arr.")";
 $query2 = "SELECT U.id,U.projectName,U.proposalDueDate,U.proposalDueTime,V.property_name,U.cust_id,U.publish,U.createdDate,U.biddingcloseddate,U.jobtype,V.timezone FROM #__cam_rfpinfo as U LEFT JOIN #__cam_property as V ON U.property_id = V.id WHERE U.rfp_type = 'closed' and  ".$condition." ORDER BY U.secondstime DESC";
...
$query4= "SELECT DISTINCT(U.id),U.projectName,U.createdDate,U.awardeddate,V.property_name,H.amount,H.awarded_vendor,U.jobtype FROM #__cam_rfpinfo as U LEFT JOIN  #__cam_property  as V ON U.property_id = V.id  LEFT JOIN  #__cam_outsidevendor  as H ON U.id = H.rfpid WHERE ".$survey_c." and (U.cust_id='".$user_id."' ".$condition.") ORDER BY U.id DESC";
	...
	$db->setQuery($query_mans);
       ...
       if($Managers_list){
		...
		$condition = " U.cust_id IN (".$totalcust_id1_arr.")";
		$query4 = "SELECT DISTINCT(U.id),U.projectName,U.createdDate,U.awardeddate,V.property_name,H.amount,H.awarded_vendor,U.jobtype FROM #__cam_rfpinfo as U LEFT JOIN  #__cam_property  as V ON U.property_id = V.id  LEFT JOIN  #__cam_outsidevendor  as H ON U.id = H.rfpid WHERE ".$survey_c." and ".$condition." ORDER BY U.id DESC";
	}
	//Completed
	
		else{
		$query4= "SELECT U.id,U.projectName,U.createdDate,U.awardeddate,V.property_name,H.amount,H.awarded_vendor,U.jobtype FROM #__cam_rfpinfo as U LEFT JOIN  #__cam_property  as V ON U.property_id = V.id  LEFT JOIN  #__cam_outsidevendor  as H ON U.id = H.rfpid WHERE ".$survey_c." and U.cust_id='".$user_id."' ORDER BY U.id DESC";
		}
...
//		$sql_award = "SELECT awardeddate,id FROM #__cam_rfpinfo WHERE rfp_type='Awarded' AND survey=0 AND cust_id=".$id ; 
	...
	function getallinsdataforindus() 
	{
		...
		if($data_geme || $data_autoins || $data_worke || $data_umbrellas || $data_lici){
		$result = 'yes';
		}
		else{
		$result = 'no';
			...
			for( $vi=0; $vi<count($vendorindustrieslist); $vi++ )
				{
					...
					if($totalprefers_new_w9 == 'success' && $totalprefers_new_gli == 'success' && $totalprefers_new_aip == 'success' && $totalprefers_new_wci == 'success' && $totalprefers_new_umb == 'success' && $totalprefers_new_pln == 'success' && $totalprefers_new_occ == 'success' ){
	...
	function vendorallindustries($vendorid){
		...
		if( $gli || $aip || $wci || $umb || $lic ){
		$finalarray = array_merge($gli, $aip, $wci, $umb, $lic);
