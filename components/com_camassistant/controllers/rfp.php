...
function save_uploadfile()
	...
	function update_rfp () {
		...
		$data['jobtype'] =  'yes';
		$data['jobnotes'] =  JRequest::getVar('jobnotes');
		}
		/*if($proposaldate[1] == '')
	   ...
	   $rfp_userid=JRequest::getVar('rfp_userid');
		...
		// To enter the exact updated date and time
		$rfpid= JRequest::getVar('rfpid');
		$updated_datetime = $model->makedatetime($rfpid);
		//echo $updated_datetime; exit;
		//Completed
		$data['update_date']=$updated_datetime;
	...
	if($model->store_update($data)) {
		...
		if(count($line_taskno)>0)
		{
			for($i=0;$i<count($line_taskno);$i++)
			{
				...
				$linetaskname[0] = preg_replace('/[^\x20-\x7E]/','', $linetaskname[0]);
	...
	//exit;
		...
		if (($_REQUEST['rfp_type']=='review')||($rfp_type=='rfp')||($rfp_type=='closed') ||($rfp_type=='Unawarded')){
			...
			if($basicrequest == 'basicrequest'){
				$msg='';
			 	$link = 'index.php?option=com_camassistant&controller=rfp&property_id='.$propertyid.'&rfp_type='.$rfp_type.'&task=reviewrfp&rfpid='.$rfp_id.'&job=basic&Itemid=199' ;
	...
	function submit_rfp()
	{
		...
		$post['create_rfptype'] = 'both';
		else
		$post['create_rfptype'] = 'personal';
		...
		//echo $updated_datetime; exit;
		//Completed
		$post['update_date']=$updated_datetime;
		...
		*/
		...
		if($basicrequest == 'basicrequest'){
		$post['jobtype'] =  'yes';
		$post['jobnotes'] =  JRequest::getVar('jobnotes');
		}
	...
	//echo "<pre>"; print_r($is_req_taskvendors); exit;
		...
		if($basicrequest == 'basicrequest'){
			...
			$mail_res = $model->sendmailtomanager($rfp_id);
			}
			else{ 
			$return_basic = $model->savebasicrequest($rfp_id,$vendorslist,'invite');
			$mail_res = $model->sendmailtomanager($rfp_id);
			}
		...
		//$this->setRedirect( 'index.php?option=com_camassistant&controller=rfp&industry_id='.$industry_id.'&property_id='.$property_id.'&show='.$show.'&rfp_type='.$post['rfp_type'].'&choose_tasks='.$choose_tasks.'&amount='.$amount.'&task=reviewrfp&rfpid='.$rfp_id.'&Itemid='.$itemid );
			...
			$this->setRedirect( 'index.php?option=com_camassistant&controller=rfp&task=successinvites&type=basic&Itemid=199' ); 
			}
			else {
	...
	function saverfpmsg()
	{
		...
		//echo "<pre>"; print_r($_REQUEST); exit;
			...
			if( $jobtype == 'yes' ){
			$model->updateproposalstodraft($rfpid,$property_id);	
			}
	...
	for( $v=0; $v<count($totalinvites); $v++ ){ 
		...
		if( $previous == 'no' ) {
	$v_company = "SELECT u.company_name, v.email, v.ccemail, v.subscribe, u.faxid, u.fax_acc  FROM #__cam_vendor_company as u, #__users as v where u.user_id=".$totalinvites[$v]->proposedvendorid." and u.user_id=v.id ";
	...
	$pdata = "SELECT u.property_name, v.projectName, v.proposalDueDate, v.proposalDueTime, v.cust_id, v.create_rfptype, v.jobtype, v.property_id FROM #__cam_property as u, #__cam_rfpinfo as v where u.id=v.property_id and v.id=".$rfpid." ";
	$db->Setquery($pdata);
	...
	$site_url = JURI::root() ;
	$link = $site_url."index.php?option=com_camassistant&controller=proposals&task=vendor_proposal_form&view=proposals&Prp_id=".$rfpdetails->property_id."&rfp_id=".$rfpid."&mot_interested=0&type=invitation&id=".$proposalid."&jobtype=".$rfpdetails->jobtype."&Itemid=112";
	...
	$cclist = explode(';',$ccemail);
		...
		$pdf->lastPage();
		$title= $rfpid.'_Invitation_'.$totalinvites[$v]->proposedvendorid.'.pdf'; //set title
		$pdf->SetTitle($title);
	...
	$db->query(); 
		...
		//Completed
		$this->setRedirect( 'index.php?option=com_camassistant&controller=rfp&task=successinvites&jobtype='.$jobtype.'&Itemid=199' );
		//JRequest::setVar('view', 'rfp');
		//parent::display();
	}
	...
	function presonalmailtovendors()
	{
	$db=&JFactory::getDBO();
	$user=&JFactory::getUser();
	$model = $this->getModel('rfp');
	...
	$manager_details = $db->loadObject();
	$man_fullname = $manager_details->name.' '. $manager_details->lastname ;
	$man_email = $manager_details->email;
	...
	$db->query(); 
		...
		//Completed
		$this->setRedirect( 'index.php?option=com_camassistant&controller=rfpcenter&task=dashboard&Itemid=125' );
...
$db=JFactory::getDBO();
 ...
 $pmanagerdetails = "SELECT id FROM #__users WHERE id='".$mid."' ";
	...
	if( $rfp_type == 'both' || $rfp_type == 'open' ) {
		...
		$vendors_bids	=	$model->getVendor_bidsinfopre($eligible_vendors,$post['rfpid']);
		...
		$total_vendors = $all_vendors;
		else if( !$all_vendors && $_REQUEST['selectdvendors'] )
		$total_vendors = $_REQUEST['selectdvendors'];
   ...
   $count = count($_REQUEST['selectdvendors']);
	...
	for($i=0; $i<$count; $i++){
		...
		$db->SetQuery($sql);
		$db->query();
		$sql_av = "UPDATE #__cam_vendor_availablejobs SET status='0' where rfp_id = ".$post['rfpid']." AND user_id= ".$post['vendorid']." "; 
		$db->SetQuery($sql_av);
		$db->query();
		$query = "DELETE FROM #__cam_rfp_uninvites WHERE rfpid=".$post['rfpid']." and vendorid=".$post['vendorid']." ";
		$db->setQuery($query);
		$db->query();
		$update_type = "UPDATE #__rfp_invitations SET mail='no' where rfpid=".$post['rfpid']." and vendorid=".$post['vendorid']." ";
		$db->setQuery($update_type);
		$db->query();
		}
		if($closeinvite == 'closeinvite') {
...
<div class="lineitem_pan" style="border:none;">
	...
	<p></p>
	<?php
	$w9data = $model->getw9data($industryid);
	...
	$master = $model->getmasterfirmaccount();
		...
		if($permission != 'yes' && $exmaster && $w9data) { $w9_main = 'checked="checked"'; $w9_class = 'styled_3';  
		?>
		<tr><td><input type="checkbox" value="yes" id="w9liability" class="<?php echo $w9_class; ?>" name="w9liability" <?php echo $w9_main; ?> /></td><td><strong>&nbsp;W9</strong></td></tr><tr height="15"></tr>
		<?php } else if($permission == 'yes' || !$exmaster) { ?>
		<tr><td><input type="checkbox" value="yes" id="w9liability" class="" name="w9liability" <?php echo $w9_main; ?> /></td><td><strong>W9</strong></td></tr>
		<tr height="15"></tr>
		<?php } else { ?>
		<tr><td><input type="checkbox" class="" disabled="disabled" /></td><td><strong>W9</strong></td></tr>
		<tr height="15"></tr>
		<?php }
		?>
		
	<?php if($permission != 'yes' && $exmaster && $generaldata) { $general_main = 'checked="checked"'; $gen_class = 'styled_3';  
	?>
	...
	?>
	<ul id="complianceclass_stand" class="pre_general" style="pointer-events:none; list-style-type:none;">
		...
		<li>Each Occurrence:&nbsp;<strong>$</strong><input type="text" value="<?php echo number_format($generaldata->each_occurrence,2); ?>" name="each_occurrence" /></li>
		<?php } ?>
		<?php if($generaldata->damage_retend){ ?>
		<li>Damage to Rented Premises:&nbsp;<strong>$</strong><input type="text" value="<?php echo number_format($generaldata->damage_retend,2); ?>" name="damage_retend" /></li>
		<?php } ?>
		<?php if($generaldata->med_expenses){ ?>
		<li>Med Expenses:&nbsp;<strong>$</strong><input type="text" value="<?php echo number_format($generaldata->med_expenses,2); ?>" name="med_expenses" /></li>
		<?php } ?>
		<?php if($generaldata->personal_inj){ ?>
		<li>Personal & Adv injury:&nbsp;<strong>$</strong><input type="text" value="<?php echo number_format($generaldata->personal_inj,2); ?>" name="personal_inj" /></li>
		<?php } ?>
		<?php if($generaldata->general_aggr){ ?>
		<li><table cellpadding="0" cellspacing="0"><tr><td>General Aggregate:</td><td>&nbsp;<strong>$</strong><input type="text" value="<?php echo number_format($generaldata->general_aggr,2); ?>" name="general_aggr" style="width:70px;" /></td>
		<?php } ?>
	...
	<ul id="complianceclass_stand" class="pre_auto" style="pointer-events:none; list-style-type:none;">
		<?php 
		...
		<li>Combined Single Limit:&nbsp;<strong>$</strong><input type="text" value="<?php echo number_format($autodata->combined_single,2); ?>" name="combined_single" /></li>
		<?php } ?>		
		<?php if($autodata->bodily_injusy_person){ ?>
		<li>Bodily Injury - Per Person:&nbsp;<strong>$</strong><input type="text" value="<?php echo number_format($autodata->bodily_injusy_person,2); ?>" name="bodily_injusy_person" /></li>
		<?php } ?>		
		<?php if($autodata->bodily_injusy_accident){ ?>
		<li>Bodily Injury - Per Accident:&nbsp;<strong>$</strong><input type="text" value="<?php echo number_format($autodata->bodily_injusy_accident,2); ?>" name="bodily_injusy_accident" /></li>
		<?php } ?>		
		<?php if($autodata->property_damage){ ?>
		<li>Property Damage - Per Accident:&nbsp;<strong>$</strong><input type="text" value="<?php echo number_format($autodata->property_damage,2); ?>" name="property_damage" /></li>
		<?php } ?>
	...
	<ul id="complianceclass_stand" class="pre_workers" style="pointer-events:none; list-style-type:none;">
		<?php 
		...
		<li>Each Accident:&nbsp;<strong>$</strong><input type="text" value="<?php echo number_format($workdata->each_accident,2); ?>" name="each_accident" /></li>
		<?php } ?>		
		<?php if($workdata->disease_policy){ ?>
		<li>Disease - Policy Limit:&nbsp;<strong>$</strong><input type="text" value="<?php echo number_format($workdata->disease_policy,2); ?>" name="disease_policy" /></li>
		<?php } ?>		
		<?php if($workdata->disease_eachemp){ ?>
		<li>Disease - Each Employee:&nbsp;<strong>$</strong><input type="text" value="<?php echo number_format($workdata->disease_eachemp,2); ?>" name="disease_eachemp" /></li>
		<?php } ?>
		...
		<li>Disease - Policy Limit: <input type="text" onKeyup="if(isNaN(parseInt(this.value))) { alert('Please enter valid number'); this.value=''; }" onClick="if(this.value == '0.00') this.value='';" onChange="javascript: add_commas_special(this,this.value);" value="" name="disease_policy" id="disease_policy" /></li>
		<li>Disease - Each Employee: <input type="text" onKeyup="if(isNaN(parseInt(this.value))) { alert('Please enter valid number'); this.value=''; }" onClick="if(this.value == '0.00') this.value='';" onChange="javascript: add_commas_special(this,this.value);" value="" name="disease_eachemp" id="disease_eachemp" /></li>
	...
	<tr><td></td><td>
		...
		<ul id="complianceclass_stand" class="pre_umbrella" style="pointer-events:none; list-style-type:none;">
		<?php if($umbrelladata->each_occur){ ?>
		<li>Each Occurrence:&nbsp;<strong>$</strong><input type="text" value="<?php echo number_format($umbrelladata->each_occur,2); ?>" name="each_occur" /></li>
		<?php } ?>		
		<?php if($umbrelladata->aggregate){ ?>
		<li>Aggregate:&nbsp;<strong>$</strong><input type="text" value="<?php echo number_format($umbrelladata->aggregate,2); ?>" name="aggregate" /></li>
		<?php } ?>	
	...
	<?php
		if($permission != 'yes'  && $exmaster && $omidata) { $omi_main = 'checked="checked"'; $class_omi = 'styled_3'; 
	?>
	<tr><td><input type="checkbox" value="yes" id="errorsomissions" class="<?php echo $class_omi; ?>" name="errorsomissions" <?php echo $omi_main; ?> /></td><td><strong>&nbsp;Errors & Omissions</strong></td></tr>
	<?php } else if($permission == 'yes' || !$exmaster) { ?>
	<tr><td><input type="checkbox" value="yes" id="errorsomissions" class="" name="errorsomissions" /></td><td><strong>Errors & Omissions</strong></td></tr>
	<tr height="15"></tr>
	<?php } else { ?>
	<tr><td><input type="checkbox" value="yes" id="errorsomissions" disabled="disabled"/></td><td><strong>Errors & Omissions</strong></td></tr>
	<tr height="15"></tr>
	<?php } ?>
	<tr><td></td><td>
		<?php if($permission != 'yes'  && $exmaster && $omidata) {  ?>
		<ul id="complianceclass_stand" class="pre_omi" style="pointer-events:none; list-style-type:none;">
		<?php if($omidata->each_claim){ ?>
		<li>Each Claim:&nbsp;<strong>$</strong><input type="text" value="<?php echo number_format($omidata->each_claim,2); ?>" name="each_claim" /></li>
		<?php } ?>		
		<?php if($omidata->aggregate_omi){ ?>
		<li>Aggregate:&nbsp;<strong>$</strong><input type="text" value="<?php echo number_format($omidata->aggregate_omi,2); ?>" name="aggregate_omi" /></li>
		<?php } ?>	
		<?php if($omidata->certholder_omi){ ?>
		<?php if($omidata->certholder_omi == 'yes'){ $waiver_omi = 'checked="checked"'; $class_certholder_omi = 'styled'; }?>
		<li><table cellpadding="0" cellspacing="0"><tr><td><input type="checkbox" value="yes" name="certholder_omi" <?php echo $waiver_omi; ?> class="<?php echo $class_certholder_omi; ?>" /></td><td> MyVendorCenter listed as Cert. Holder</td></tr></table></li>
		<?php } ?>	
		</ul>
		<?php } elseif($permission == 'yes' || !$exmaster) { ?>
		<ul class="pre_omi" style="list-style-type:none; display:none; margin-top:-5px;">
		<li>Each Claim: <input type="text"  onKeyup="if(isNaN(parseInt(this.value))) { alert('Please enter valid number'); this.value=''; }" onClick="if(this.value == '0.00') this.value='';" onChange="javascript: add_commas_special(this,this.value);"value="" name="each_claim" id="each_claim" /></li>
		<li>Aggregate: <input type="text" value=""  onKeyup="if(isNaN(parseInt(this.value))) { alert('Please enter valid number'); this.value=''; }" onClick="if(this.value == '0.00') this.value='';" onChange="javascript: add_commas_special(this,this.value);"name="aggregate_omi" id="aggregate_omi" /></li>
		<li><table cellpadding="0" cellspacing="0"><tr><td><input type="checkbox" value="yes" name="certholder_omi" class="" /></td><td>MyVendorCenter listed as Cert. Holder</td></tr></table></li><br />
		</ul>
		<?php } else{ }?>
	</td></tr>
...
<div class="clear"></div>
	...
	function saveinsurance($_REQUEST,$rfpid){
		...
		$w9liability = JRequest::getVar('w9liability','');
		...
		$model = $this->getModel('rfp');
			if($w9liability == 'yes'){
				...
				$db=&JFactory::getDBO();	
				$query_d = "Delete FROM `#__cam_master_w9_standards_rfps`  WHERE masterid =".$user->id." and rfpid=".$rfpid." ";
				$db->setQuery($query_d);
				$db->Query();
			}
		if($w9liability == 'yes'){
				$check = $model->getcheckw9inc($rfpid);
					if($check) { 
						$w9['id'] = $check ;
		...
		//Save errors and omissions insurance documents
		...
		if( $errorsability == 'yes' ){
			...
			$db=&JFactory::getDBO();
			$query_d = "Delete FROM `#__cam_master_errors_omissions_rfps` WHERE masterid =".$user->id." and rfpid=".$omi['rfpid']." ";
			$db->setQuery($query_d);
			$db->Query();
		}
		if($errorsability == 'yes'){
				$check = $model->getcheckomiinc($omi['rfpid']);		
					if($check) { 
						$omi['id'] = $check ;
		...
		<a class="uploadimagetext" href="javascript:void(0);" onclick="getpopuppics(<?php echo $pid; ?>);"> UPLOAD PROPERTY IMAGE</a></div></div>
...
window.parent.document.getElementById('imagediv').innerHTML = '<img width=".$thumb_width." height=".$thumb_height." src=\'components/com_camassistant/assets/images/property_pictures/".$filename."\' />';
	...
	function invitenewvendor(){
		$model	=& $this->getModel('rfp');
		$vendorslist = JRequest::getVar('invitedvendors','');
	...
	function rfpapprovalcheck()
	{
	$db =& JFactory::getDBO();
	$property_id = JRequest::getVar('property_id','');
	$query_email="SELECT id FROM #__cam_property WHERE id='".$property_id."' AND approval_request=1 AND property_link=1 ";
