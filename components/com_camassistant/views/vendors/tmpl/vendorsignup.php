...
class vendorsignupController extends JController
{
	...
	function save_billing_info()
	{
		...
		$_SESSION[vendor][$key]=$value;
				...
				$_SESSION['vsignup']['ccemail']= $_REQUEST['ccemail'];
				
				$_SESSION['vsignup']['bill_companyname']= $_REQUEST['bill_companyname']; 
				$_SESSION['vsignup']['company_phone']= $_REQUEST['company_phone1'].'-'.$_REQUEST['company_phone2'].'-'.$_REQUEST['company_phone3']; 
				$_SESSION['vsignup']['faxid']= $_REQUEST['fax_1'].'-'.$_REQUEST['fax_2'].'-'.$_REQUEST['fax_3']; 
				$_SESSION['vsignup']['fax_acc']= 'yes';
				...
				$_SESSION['vsignup']['subscribe']= $_REQUEST['subscribe']; 
				$_SESSION['vsignup']['manager_invitecode']= $_REQUEST['manager_invitecode']; 
		...
		//exit;
		$post['company_phone'] = $post['company_phone1'].'-'.$post['company_phone2'].'-'.$post['company_phone3'];
		$post['faxid'] = $post['fax_1'].'-'.$post['fax_2'].'-'.$post['fax_3'];
		$post['fax_acc'] = 'yes';
		...
		$_SESSION['vendor']['company_phone']				= $post['company_phone1'].'-'.$post['company_phone2'].'-'.$post['company_phone3'];
		$_SESSION['vendor']['faxid']						= $post['fax_1'].'-'.$post['fax_2'].'-'.$post['fax_3'];
		$_SESSION['vendor']['fax_acc']						= $post['fax_acc'];
...
$db =& JFactory::getDBO();
	for($j=0;$j<count($data['county']);$j++) 
	{
		...
		$county_id = $stateandcounty[1];
			...
			if($county_id == 'All'){
			$select_all = "SELECT * FROM #__cam_counties WHERE State='".$state_id."' ORDER BY County";
			$db->Setquery($select_all);
			$allcounties = $db->loadObjectList();
			for($c=0; $c<=count($allcounties); $c++){
			$query_all = "insert into #__vendor_state_counties (`id` ,`user_id` , `state_id` ,`county_id`) VALUES ('','".$ind_data['user_id']."','".$state_id."','".$allcounties[$c]->id."')";
		...
		$db->query();
					...
					$invitedemail_id="SELECT vid FROM #__vendor_inviteinfo where inhousevendors='".$data['email']."'";
					$db->Setquery($invitedemail_id);
					$email_invited = $db->loadResult();
					if($email_invited){
					$sql_update = "UPDATE #__vendor_inviteinfo SET v_id='".$ind_data['user_id']."' where inhousevendors='".$data['email']."'"; 
					...
					//Insert as preferred vendor to the invite code manager
		...
		//Completed
					...
					//$link = "<a href='myvendorcenter.com/cms/index.php'>CLICK HERE</a>"; 
		...
		//			$mailfrom = 'cam@company.com';
					...
					$cc = 'vendoremails@myvendorcenter.com';
					...
					$db->Setquery($mail_content_sql);
					$content = $db->loadResult();
					$body = str_replace('{VENDOR NAME}',$company['company_name'],$content) ;
		...
		//$body = "Hi ".$user['name'].'&nbsp;'.$user['lastname']."! <br/>Please <a href=".$siteURL."index.php?option=com_camassistant&controller=vendors&Itemid=158&task=mail_redirect_form&useractivation=".$vendor['activation']."&view=vendors'>CLICK HERE</a> to complete your Vendor Registration.<br/><br/>At Your Service,<br/>The<a href=".$siteURL.">CAMassistant.com </a>Team";
			...
			//echo count($post['county']);
			 $stateandcounty = explode("_",$post['county'][0]);
			
		$state_id = $stateandcounty[0];
		$county_id = $stateandcounty[1];
		
		if($county_id == 'All'){
			$maincounty='All Counties';
		} else {
		...
		//Completed	
				...
				$res = JUtility::sendMail($mailfrom, '', $to_sateesh, $subject, $msg, $mode=1);
...
function verfiryinvitecode(){
	...
	$codes="SELECT id FROM #__users where manager_invitecode='".$invite."'"; 
	$db->Setquery($codes);
	$code_details = $db->loadResult();
		if($code_details){
			echo "notused";
		}
		else{
