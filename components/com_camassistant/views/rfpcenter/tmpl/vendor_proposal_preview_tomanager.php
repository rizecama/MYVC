...
<div id="i_bar_basicinfo">
<div id="i_bar_txt" style="text-align:center; width:643px; font-size:14px;">
<span style="padding-left:0px; color:#fff;"><strong>SPECIAL REQUIREMENTS</strong></span></div>
</div>

<div class="lineitem_pan_manager">
<?php
...
<a href="index.php?option=com_camassistant&controller=popupdocs&task=open&title=<?php echo str_replace(' ','',$uploads1[$upcount-1]); ?>&path=<?php print_r($file_link); ?> "><img src="templates/camassistant_left/images/green.png" alt="download file" align="absmiddle" /></a></span></td>&nbsp;&nbsp;&nbsp;<td><span class="showfile_manager"><?php   $upcount=count($uploads1); ?> <?php echo str_replace('_',' ',$uploads1[$upcount-1]); ?></span></td></tr>
<?php } } }  ?>
<!-- eof line item pan -->
<?PHP } ?></table>
<div class="clear"></div>
...
<!--  This is for vendor response -->
<div id="i_bar_basicinfo_response">
<div style="text-align:center; width:643px; font-size:14px;" id="i_bar_txt">
...
<?PHP } else { ?>
<span class="linenotes_vendor_response" style="float:left; width:628px; font-size:14px;">Line Item #<?PHP echo $i+1; ?>: <?PHP echo $TASKS[$i]->linetaskname; ?>
<span class="managerside_price"><?PHP if($TASKS[$i]->is_req_taskvendors == 1) { $price_cnt = $price_cnt+1;?>
<?PHP if($TASKS[$i]->price != '' && $TASKS[$i]->price!='(Line-Item Pricing)') { ?>$<?PHP echo $TASKS[$i]->price; ?> <?PHP } else echo '-----'; ?>
<?PHP } ?></span>
...
<strong>Exception:</strong><p style="padding-left:0px;"> <?PHP if($TASKS[$i]->exception_desc[0] != '' && $TASKS[$i]->exception_desc[0]!='(list all Exceptions here)') echo $TASKS[$i]->exception_desc; else echo '-----'; ?>
    <?php echo "<br />"; } ?>
</div>	
 <table><div class="downloadfile_manager">  <?Php $files_cnt = count($TASKS[$i]->uploaded_file);
...
<!--  This is for vendor response completed-->
<!--  sof line item pan -->
