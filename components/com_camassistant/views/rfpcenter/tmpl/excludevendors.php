...
H(document).ready( function(){
	H('#acceptlink').click(function(){
		H.post("index2.php?option=com_camassistant&controller=vendorscenter&task=blockvendors", function(data){
...
<p style="font-size:13px;">Vendors who choose a basic membership will NOT have their Compliance Documents verified for accuracy by the MyVendorCenter compliance team and will appear as <font color="#FF0000">UNVERIFIED</font>. This means there is a chance the vendor has uploaded an incorrect document, expired poliy, or provided incorrect information (i.e policy amount, types of coverage). We highly recommend you verify all information entered by these vendors manually before inviting them to projects. You can also choose to block unverified vendors from being invited to projects by your managers by selecting the option below.</p>
<br />
<div align="center"><input type="checkbox" <?php if( $block ) echo "checked='checked'";?> id="acceptlink" name="accept"  style="margin-top: -1px; vertical-align: middle;" />
  ...
  <div id="preferredvendorsinvitations">
   ...
   <input type="checkbox" value="<?php echo  $am->vid; ?>" name="coworkers" class="coworkers" />
	...
	<div class="search-panel-image_rfp compliant_vendor">
		 ...
		 <?php  if($globe != 'fail'){ ?>
			<a href="index.php?option=com_camassistant&controller=vendorscenter&task=preferredcompliance&vendorid=<?php echo $am->id; ?>" rel="{handler: 'iframe', size: {x: 650, y: 800}}" class="modal" id="<?php echo $id; ?>" title="<?php echo $title; ?>"><?php echo $text; ?></a>
			<?php } else {
			echo "N/A";
			}?>			
			 </p>
...
<div class="unverifiedvendor"><a href="javascript:void(0);" onclick="unverified(<?php echo $am->id; ?>,'unverified');">UNVERIFIED</a></div>
<?php } else {  ?>
<div class="verifiedvendor"><a href="javascript:void(0);" onclick="unverified(<?php echo $am->id; ?>,'verified');">VERIFIED</a></div>
<?php } ?>
