...
function getnormalpopup(matchesa){
		...
		L('#maskv').hide();
		L('.windowv').hide();
				H.post("index2.php?option=com_camassistant&controller=vendorscenter&task=removevendor_list", {vendorid: ""+matchesa+""}, function(data){
				if(data){
				location.reload(); 
				}
				else{
...
alert("Please make a selection to ADD the vendors.");
	...
	L(".coworkers:checked").each(function() {
			...
			matchesb = matchesar.join(',') ;
			prevendors = 1;
			
		L.post("index2.php?option=com_camassistant&controller=vendorscenter&task=check_preferredvendor",{vendorid: ""+matchesb+""}, function(data){
		
		if(data==1)
		geterror_popup();
		else
		{
	L.post("index2.php?option=com_camassistant&controller=vendorscenter&task=addmyvendortopreferredvendor&prevendor="+prevendors+"",{vendorid: ""+matchesb+""},              function(data){
				if(data){
...
<tr><td>    
	...
	<input type="hidden" name="task" value="mastermyvendors" />
...
?>
 <p style="height:3px;"></p>
  ...
  <div align="center" class="totalvendorspre_preferred">
<?php 
$items = $this->items;
...
?>		 
  ...
  <div id="preferredvendorsinvitations">
   <div class="search-panel-middlepre checkbox_vendor">
	  ...
	  <input type="checkbox" value="<?php echo  $am->vid; ?>-<?php echo  $am->v_id; ?>" name="preferred" class="preferredvendors<?php echo $final_display; ?>" style="margin-left:-15px;" />
	   <?php } 
	  else { ?>
...
<?php } ?>
	...
	<a title="Add to Corporate Preferred Vendors" class="addpreferredvendoricon" href="javascript:sendpreferredvendor_invitation();"></a>
...
<div style="text-align:justify"><p class="Corporate_status">This Vendor is already a Corporate Preferred Vendor</p>
</div>
