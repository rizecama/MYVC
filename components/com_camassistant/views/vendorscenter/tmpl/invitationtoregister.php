...
<div class="green-arrow-newclient">&nbsp;</div>
<?php 
$properties = $this->propertyList; 

?>
 
<select id="property_name" name="property_name" disabled="disabled" style="width:100%; height:38px; padding:5px; border: 1px solid #6d6d6d;">

	<?php
		for( $p=0; $p<count($properties); $p++ ){
		if($properties[$p]->value == $propertyid) 
		...
		<option <?php echo $selected; ?>  value="<?php echo $properties[$p]->value; ?>" title="<?php echo $property;?>"><?php echo $propertynew; ?></option>
		<?php }
	?>
	</select>
</div>
...
<input type="text" style="max-width: 238px; padding-left:88px; width: 100%; color:gray; margin-right: 10px; height: 29px;"  id="email" name="email" value="<?php echo $getemail;?>" readonly="readonly">
</div>
</div>
...
<input type="hidden" value="<?php echo $boardmemeberid; ?>" name="bid">
<input type="hidden" value="<?php echo $propertyid; ?>" name="propertyid">
<input type="hidden" value="<?php echo $getemail1; ?>" name="email">

<input type="hidden" value="sendregisterinvitation" name="task">
</form>
