...
$rfpdate = $this->rfp_data; 
if( $rfpdate->create_rfptype == 'both' ){
	$selected_both = 'checked="checked"';
	$selected_open = 'checked="checked"';
	}
else if( $rfpdate->create_rfptype == 'open' )	
	$selected_open = 'checked="checked"';
else
	$selected_open = '';	
 ?>
...
H(document).ready( function(){
	H('.open_request').click(function(){
		//H(".sponsored").attr("checked", true);
		//H('#preferred').hide();
		if( H('input[name=open]:checked').val() )
			H(".rfp_type_open").val('open');
		else
			H(".rfp_type_open").val('');
	});
	H('.personal_request').click(function(){
		//H(".sponsored").attr("checked", false);
		H('#preferred').show();
		if( H('input[name=personal]:checked').val() )
			H(".rfp_type_personal").val('personal');
		else
			H(".rfp_type_personal").val('');	
	});
...
var preinvites = '<?php echo $count_inv; ?>' ;
 str = '';
 if (!PR('input[name=open]:checked').val() && !PR('input[name=personal]:checked').val() && closei != 'closeinvite' ) {  
			 geterrorpopup();      
		}
else{		
...
function geterrorpopup(){
	...
	var maskHeight = PR(document).height();
	var maskWidth = PR(window).width();
	PR('#maskvrecdonee').css({'width':maskWidth,'height':maskHeight});
	PR('#maskvrecdonee').fadeIn(100);
	PR('#maskvrecdonee').fadeTo("slow",0.8);
	var winH = PR(window).height();
	var winW = PR(window).width();
	PR("#submitvrecdonee").css('top',  winH/2-PR("#submitvrecdonee").height()/2);
	PR("#submitvrecdonee").css('left', winW/2-PR("#submitvrecdonee").width()/2);
	
	PR("#submitvrecdonee").fadeIn(2000);
	PR('.windowvrecdonee #closevrecdonee').click(function (e) {
		e.preventDefault();
		PR('#maskvrecdonee').hide();
		PR('.windowvrecdonee').hide();
	});
}
...
var options = $merge(options || {}, Json.evaluate("{handler: 'iframe', size: {x: 700, y:"+height+"}}"))
SqueezeBox.fromElement(el,options);
}
...
//print_r($blockedvendors);exit;
?>
<p style="height:40px;"></p>
<div class="toppart_header">
<ul class="toppart_ul">
<li><img src="templates/camassistant_left/images/step2.jpg" /></li>
<li class="basic_heading">How would you like to invite Vendors to participate in your request?</li>
</ul>
<ul class="input_options">
<li>
<?php if( $blockedvendors->block == 1 || $blockedvendors->block_complinace == 1 ) { ?>
<input type="checkbox" name="open" value="open" class="block_unverified" <?php echo $selected_both; ?> <?php echo $selected_open; ?> /> - <span>Open Invitation:</span> I would like my request to be open to ALL interested Vendors<span> (<a href="javascript:void(0);" onclick="javascript:getopenpopuptext();">more info</a>)</span>
<?php } else { ?> 
<input type="checkbox" name="open" value="open" class="open_request" <?php echo $selected_both; ?> <?php echo $selected_open; ?> /> - <span>Open Invitation:</span> I would like my request to be open to ALL interested Vendors<span> (<a href="javascript:void(0);" onclick="javascript:getopenpopuptext();">more info</a>)</span>

<?php } ?>
</li>
<li><input type="checkbox" name="personal" value="personal" class="personal_request" <?php echo $selected_both; ?> <?php echo $selected_open; ?> /> - <span>Personal Invitation:</span> I would like to select specific Vendors to invite to my request<span> (<a href="javascript:void(0);" onclick="javascript:getpersonalpopuptext();">more info</a>)</span> </li>
</ul>
</div>
<?php } ?>

<?php
if($_REQUEST['var'] == 'closeinvite' || $rfp_type == 'rfp')
$disp = '';
else
$disp = 'none';
?>
<div id="preferred" style="display:<?php echo $disp; ?>;" >
<?php if($close != 'closeinvite'){ ?>
...
<p style="margin-top:5px;">Note: If you do NOT see enough, or any, Vendors on this page, we suggest you revise the information (industry, insurance + licensing requirements) entered in STEP 1 and/or personally <a class="hoverinvitevendors" href="/index.php?option=com_camassistant&amp;controller=vinvitations&amp;Itemid=216">INVITE VENDORS</a> to join MyVendorCenter.com.</p>
<div class="clr"></div>
</div>
<?php } ?>
<div id="add-vendor">
...
<?php
	...
	if( $am->unverified == 'hide' && $am->block_nonc == 'hide' )
		{
			if( ( $am->subscribe_type == 'free' || $am->subscribe_type == '' ) && $am->special_requirements == 'fail' ){
			$args = 'both';
			}
			else if( ( $am->subscribe_type == 'free' || $am->subscribe_type == '' ) && $am->special_requirements == 'success' ){
			$args = 'un';
			}
			else if( ( $am->subscribe_type != 'free' || $am->subscribe_type != '' ) && $am->special_requirements == 'fail' ){
			$args = 'nonc';
			}
...
?>			 
 	...
 	<div id="preferredvendorsinvitations" style="display:<?php echo $display; ?>">
	...
	<div class="search-panel-image_rfp compliant_vendor">
	  <?php 
	  		...
	  		if($existing_stand == 'no'){
				...
				if($am->special_requirements == 'fail' || $am->acount_type == 'show' ) {
	  ...
	  <p align="center" style="color:<?php echo $spe_color; ?>; display:block; margin-bottom:7px; font-weight:bold; padding-right:0px;">
	   <?php if($globals != 'fail'){ ?>
			<a href="javascript:void(0);" id="<?php echo $compliant_id; ?>" onclick="getstandards('<?php echo $rfpid; ?>','<?php echo $am->v_id; ?>','<?php echo $title; ?>');" title="<?php echo $title ; ?>"><?php echo $images_text ; ?></a>
			<?php } else {
			echo "N/A";
			}?>
			 </p>
...
<div id="i_bar_terms_rfp_yellow" style=" background: #e6b613; box-shadow: 1px 2px 1px #808080; height: 34px; margin-bottom: 10px; text-align: center;">
<div id="i_bar_txt_terms_rfp">
...
?>	
 	...
 	<div id="preferredvendorsinvitations" style="background:<?php echo $background; ?>; display:<?php echo $display; ?>">
   <div class="search-panel-middlepre checkbox_vendor">
    <?php if($am->pre == 'yes'){ $checked = 'checked'; } else { $checked = ''; } ?>
    <?php if($am->vendorstatus == 'eligible' && $am->vendorstatus != 'suspend'){ ?>
		<?php 
	...
	<div class="search-panel-image_rfp compliant_vendor">
	 ...
	 <p align="center" style="color:<?php echo $spe_color; ?>; display:block; margin-bottom:7px; font-weight:bold; padding-right:0px;">
	 <?php if($globals != 'fail'){ ?>
			<a href="javascript:void(0);" id="<?php echo $compliant_id; ?>" onclick="getstandards('<?php echo $rfpid; ?>','<?php echo $am->id; ?>','<?php echo $title; ?>');" title="<?php echo $title ; ?>"><?php echo $images_text ; ?></a>
...
<a href="index.php?option=com_camassistant&controller=rfpcenter&task=dashboard&Itemid=125" class="cancel_rfp_invite"></a></td>&nbsp;&nbsp;&nbsp;<td>
...
<div align="center">
<table cellpadding="0" cellspacing="0"><tr><td>
<a class="goback_standardrfp" href="index.php?option=com_camassistant&controller=rfp&task=editrfp&rfpid=<?php echo $_REQUEST['rfpid']; ?>&var=draft&Itemid=87"></a></td>&nbsp;&nbsp;&nbsp;<td>
...
<?php } ?>
	...
	<input type="hidden" name="rfptype" id="rfptype" value="" />
	<input type="hidden" name="personal" value="" class="rfp_type_personal" />
	<input type="hidden" name="open" value="" class="rfp_type_open" />
	</form>
