...
//       K("#loading-div-background").css({ opacity: 1.0 });
...
<span style="margin-left:53px; text-transform: uppercase;"><strong><?php echo $rfpinfo->projectName; ?></strong></span></div>
<div id="i_icon"><?php
 $user = JFactory::getUser();
...
<span><strong>REQUEST #<?php echo $_REQUEST['rfpid']; ?></strong></span>
</div>
</div>
...
<div class="signup2" style="width:100%;">
<label>
...
<div  style="border-top:1px solid #CDCDCD;  padding: 4px 0; text-align:left" ><strong>Description: </strong><?php 
$html = nl2br($linetaks->task_desc) ;
...
<tr><td>
<a href="index.php?option=com_camassistant&controller=popupdocs&task=open&title=<?php echo str_replace(' ','',$linetaks->taskuploads); ?>&path=<?php print_r($file_link); ?> "><img src="templates/camassistant_left/images/green.png" alt="download file" align="absmiddle" /></a></td><td><?php   $upcount=count($uploads1); ?> <span style="color:#8FD800"><?php echo $uploads1[$upcount-1]; ?></span></td></tr>
<?php } } } } ?></table>
</div>
<?php // } ?>
...
<table cellpadding="0" cellspacing="0" border="0">
	<?php
	if( $w9data ) { $w9_main = 'checked="checked"'; $w9_class = 'styled';  
	?>
	<tr><td><input type="checkbox" value="yes" id="w9document" class="<?php echo $w9_class; ?>" name="w9document" <?php echo $w9_main; ?> /></td><td><strong>W9</strong></td></tr>
	<?php } ?>
	...
	<ul class="pre_general" style="pointer-events:none; list-style-type:none;">
		...
		<td valign="bottom" style="padding-top:3px;">General Aggregate:&nbsp;</td><td valign="bottom"><span id="greenbolddolor"><?php echo "$".number_format($aggregate_price,2); ?></span> </td>
		<?php } ?>
		...
		<td valign="top" style="padding-top:2px;"><input type="radio" name="applies_to" value="loc" <?php echo $loc; ?> /></td><td valign="top" style="padding-top:2px;"> Loc</td>
		<?php } ?>
		</tr></table></li>
...
<?php if($autodata->combined_single && $autodata->combined_single != '0.00'){ ?>
	...
	?>
	<tr><td><input type="checkbox" value="yes" id="omirellaliability" class="<?php echo $class_omi; ?>" name="omirellaliability" <?php echo $omi_main; ?> /></td><td><strong>ERRORS & OMISSIONS INSURANCE</strong></td></tr>
	<?php } ?>
	<tr><td></td><td>
...
$ratecount = "SELECT V.apple FROM `#__cam_vendor_proposals` as U, `#__cam_rfpinfo` as V where U.proposedvendorid=".$am->vendorid." and V.apple!=0 and V.apple_publish=0 and U.proposaltype='Awarded' and U.rfpno = V.id ";
	...
	<div class="search-panel-image_rfp_inv compliant_vendor_inv" style="margin-top:0px; padding-top:0px;">
	  <?php 
	...
	<a href="javascript:void(0);" id="<?php echo $compliant_id; ?>" onclick="getstandards('<?php echo $rfpinfo->id; ?>','<?php echo $am->vendorid; ?>','<?php echo $title; ?>');" title="<?php echo $title ; ?>"><?php echo $images_text ; ?></a>
