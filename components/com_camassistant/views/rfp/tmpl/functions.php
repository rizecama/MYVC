	...
	function RSfrontComponentBody($formId,$componentId,$data,$value='')
	{
		...
		switch(RSresolveComponentTypeId($r['ComponentTypeId']))
		{
			...
			case 'endLineItemTask':
			{
				...
				$out .= '<div><div style="float:left;width:215px;">Frequency:<input type="text"  value=""  name="form[fr'.$frequency.']" id="fr'.$frequency.'" /></div>';
$out .= '<div style="float:left;width:175px;"><select class="frsel'.$frequency.'" id="frsel'.$frequency.'" name="form[frsel'.$frequency.'][]"><option value="PLEASE SELECT">PLEASE SELECT</option><option value="WEEK">WEEK</option><option value="MONTH">MONTH</option><option value="YEAR">YEAR</option></select> (OR) </div>';
$out .= 'Enter custom Frequency Below:<br/><textarea name="form[frother'.$frequency.']" id="froher'.$frequency.'"  cols="30" style="background:none; height:35px;margin-top:0px;margin-left:2px;"></textarea></div>';
	...
	function RSprocessForm($formId)
	{
		...
		foreach ($_POST['form'] as $key => $val)
		{
			...
			if(($val)&&($val!='PLEASE SELECT')){
				...
				AND rp.PropertyValue = '".$key."'
				AND rc.ComponentId = rp.ComponentId
				AND rsv.FormId = rc.FormId group by ComponentId  having count(*)>=1)";
				...
				if($linecaption){
				if(!strstr($linecaption,'{n/a}')){
				$linetask.=$linecaption.':'.$val.',';
				}
				}else{
				$linetask.=$val.',';
				}
				...
				unset($frequency);
				$frequency=$val;
				$linetask=eregi_replace($val,'',$linetask);
				}
				/***********End Frequency Text ***************/
				...
				unset($frequencysel1);
				$frequencysel1=$val;
				$frequency=$frequency.$frequencysel1;
				$linetask=eregi_replace($val,'',$linetask);
				}
				...
				$linetask=str_replace('submit:','',$linetask);
				$line_title=$val;
				if($i>1){
				$required_vendor=1;
				}
				if($linetask){
				$db->setQuery("INSERT INTO #__rsform_submission_values SET `SubmissionId`='".$SubmissionId."',Frequency='".$frequency."', `FormId`='".$_POST['form']['formId']."',`line_title`='".$line_title."',`required_vendor`='".$required_vendor."', `FieldName`='".$key."', `FieldValue`='".strip_tags($linetask)."'");
				$db->query();
				}
				$i++;
				$j++;
