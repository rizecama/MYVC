...
$propertyownerinfo=$this->prouserinfo;
...
<script type="text/javascript" src="<?php echo Juri::base(); ?>components/com_camassistant/skin/js/jquery.textarea-expander.js"></script>
	...
	function removeEvent1(divid)
	{
		...
		if(res==true)
		{
			...
			G('.totalblocks').each(function(){
				...
				if(i == 1){
				G(this).find('.linetasknumbers').val("210");
				G(this).find('.newdivaclass').attr("id","newdiva210");
				G(this).find('.upload_new_files_rfp').attr("onclick","addEventa2(2)");
				G(this).find('.other_tfield').attr("name","linetaskname_210[]");
				G(this).find('.other_tfield_desc').attr("name","task_desc_210[]");
				}
				else{
				G(this).find('.linetasknumbers').val("2"+i);
				G(this).find('.newdivaclass').attr("id","newdiva2"+i);
				G(this).find('.upload_new_files_rfp').attr("onclick","addEventa2("+i+")");
				G(this).find('.other_tfield').attr("name","linetaskname_2"+i+"[]");
				G(this).find('.other_tfield_desc').attr("name","task_desc_2"+i+"[]");
				}
...
<div align="center" class="processing_image"><img src="templates/camassistant_left/images/step1.jpg"></div>
...
?>
	...
	<div id="rpf_ifo_text"><h1 style="line-height:20px;">Property Owner:</h1>
	<?php //echo "<pre>"; print_r($RFP); ?>
	<?php 
	$db =& JFactory::getDBO();
	 ?>
<span><?PHP echo $RFP->comp_name;  ?></span>
<?PHP echo $propertyownerinfo->steetaddress;  ?><br />
<?PHP echo $propertyownerinfo->city;  ?>, <?PHP echo $propertyownerinfo->state?> <?PHP echo $propertyownerinfo->zipcode;  ?><br />
...
<p id="hiddenmanager"><?PHP echo $user->name;  ?> <?PHP echo $user->lastname;  ?><br /></p>
<p id="manager_name"></p>
<p id="hidephone">P: <?PHP echo $user->phone;  ?> <?PHP if($user->extension) {echo "(x".$user->extension.")";} else{}  ?></p>
<p id="showphone"></p>

<br />
<a href="mailto:<?PHP echo $user->email;  ?>">Email Property Manager</a>
<br /><br />
...
<!--eof rfp box--><!--<span style="margin-left:5px">DEFINE YOUR RFP DETAILS BELOW:</span>-->
 ...
 <div class="duplicate_image"><p>Please select a property below. Once selected, your property image will appear here.</p></div>
 </div>
 <div id="imagediv"></div>
 <br />
...
<option value="<?php echo $properties->value; ?>">
<?php 
	if( $properties->origin == '' )
		$propertyname =  str_replace('_',' ',$properties->text);
	else
		$propertyname =  $properties->origin;	
	echo 	$propertyname;
		 ?> 
</option>
<?php }  ?>
			...
			if((inputs[a].type == "checkbox" || inputs[a].type == "radio") && inputs[a].className == "styled_3") {
				...
				if(inputs[a].checked == true) {
					if(inputs[a].type == "checkbox") {
						position = "7px -" + (checkboxHeight*2) + "px";
...
<tr><td><td><p style="margin-left: 7px;">Sealed bidding allows you to see a Vendor’s quote (pricing) ONLY after you manually close the RFP.</p></td></tr>
</table>
</div>
</div>
<br />
...
<?php echo JHTML::_('select.genericlist', $industryList, 'industry_id', 'size="1" style="width:300px;" class="inputbox" onchange="firstindustry(this.value);""', 'value', 'text', '');?>
</div>
<br />
<div class="signup">
<div class="state">
...
<p class="industry_not_selected">Please select an Industry to display your Company's Compliance Standards</p>
...
<textarea class="other_tfield_desc" id="line_taskdesc1" name="task_desc_210[]"></textarea>
<span class="redstarnew">*</span>
...
<input type="hidden" checked="" id="line_taskno" class="linetasknumbers" name="line_taskno[]" value="210" >
...
<div class="clear"></div>
<div id="newdiva210" class="newdivaclass"></div>
</div>
</div>
<br />
</div>
