...
function doc_upload_general(id){
	el='<?php  echo Juri::base(); ?>index2.php?option=com_camassistant&controller=proposals&task=upload_select&warranty=general_popup&taskid='+id;
	var options = $merge(options || {}, Json.evaluate("{handler: 'iframe', size: {x: 733, y:450}}"))
	SqueezeBox.fromElement(el,options);
}
...
SqueezeBox.fromElement(href,options);
	...
	var id2_general=document.getElementById('idvals_general').value;
	var endid2_general;
	function addEvent2_general(taskid_general,tid_general)
	{
			id2_general++;
			var arrlicen2_general=new Array();
			var ni2_general = document.getElementById('newdiv_general'+taskid_general);
			var numi2_general = document.getElementById('theValues_general');
			var num2_general = (document.getElementById("theValues_general").value -1)+ 2;
			var add_general = taskid_general+''+id2_general;
			numi2_general.value = num2_general;
			var divIdName2_general = "upload_general"+num2_general;
			newitem2_general='<div style="padding:0px 0px 0px 5px;"><table><tr><td><span id="delimg_general'+id2_general+''+num2_general+'" style="display:none" title="Remove From RFP"><img src="<?php echo Juri::base(); ?>templates/camassistant_left/images/red.png" alt="delete" style="cursor:pointer;" onclick="javascript:removeEvent2_general('+num2_general+','+taskid_general+');"/></span></td><td><span id="upload_file_general'+id2_general+''+num2_general+'" style="float:left;font-size:14px;width:auto;padding-right:5px;"></span></td><input type="hidden" name="hid_text_general'+add_general+'" id="hid_text_general'+id2_general+''+num2_general+'" value="" /><input name="fpath_general'+tid_general+'[]" id="fpath_general'+id2_general+''+num2_general+'" type="hidden" value="" /></div></tr></table><div class="clear"></div>';
			var newdiv_general = document.createElement('div');
			newdiv_general.setAttribute("id",divIdName2_general);
			newdiv_general.innerHTML = newitem2_general;
			ni2_general.appendChild(newdiv_general);
            doc_upload_general(id2_general+''+num2_general);
	}
...
var id2 =  '';
	...
	function addEvent2(taskid,tid)
	{
		...
		{ */
			...
			newitem2='<div style="padding:0px 0pt 0px 5px;"><table><tr><td><span id="delimg'+id2+''+num2+'" style="display:none" title="Remove From RFP"><img src="<?php echo Juri::base(); ?>templates/camassistant_left/images/red.png" alt="delete" style="cursor:pointer;" onclick="javascript:removeEvent2('+num2+','+taskid+');"/></span></td><td><span id="upload_file'+id2+''+num2+'" style="float:left;font-size:14px;width:auto;padding-right:5px;"></span></td><input type="hidden" name="hid_text'+add+'" id="hid_text'+id2+''+num2+'" value="" /><input name="fpath_'+tid+'[]" id="fpath'+id2+''+num2+'" type="hidden" value="" /></div></tr></table><div class="clear"></div>';
...
function removeEvent2_general(divNum2_general,taskid_general)
	{
			//divNum2 = document.getElementById("theValues").value;

		if(divNum2_general != 0 )
		{
			var divNum3_general=divNum2_general-1;
			//var d2=taskid;
			//alert(divNum3);
			var d2_general = document.getElementById('newdiv_general'+taskid_general);
			var olddiv2_general = document.getElementById("upload_general"+divNum2_general);
			d2_general.removeChild(olddiv2_general);
			var numi2_general = document.getElementById('theValues_general');
			var num2_general = document.getElementById("theValues_general").value -1;
			numi2_general.value = num2_general;
		}
		id2_general--;
	/*if(id1<=cnt)
...
P = jQuery.noConflict();
 P('textarea[rel="editor"]').each(function(){
  var n=P(this).attr('id');
	...
	notesstrings = notesstrings.replace(/[^\u000A\u0020-\u007E]/g, ' ');
	notesstrings = notesstrings.replace(/<img[^>]*>/g,"");
...
function deletegeneralfiles(pid,ptype,rfpid,docid){
	var cnf = confirm("Are you sure you want to delete this file ?");
	if( cnf  == true){
		E.post("index2.php?option=com_camassistant&controller=proposals&task=remove_genfile", {prid: ""+pid+"",rfp_id: ""+rfpid+"",protype:""+ptype+"",fileid:""+docid+""}, function(data){
		if( data == 'deleted' ){
			E('.gendos_'+docid).hide();
		}
		else{
			alert("Not able to delete this file");
		}	
		});
	}
	else{
...
<!-- starts top menu -->
<?php /*?><div align="center" id="instructionsprint">
...
<br/><div readonly="readonly" cols="23" style="width:197px; text-align:left; padding:3px; font-family:arial; font-size:12px; color:#7D7D7D; border:1px solid #D7D7D7; background:#F7F7F7; margin-top:9px; resize:vertical;"><?PHP echo $RFP->bidders_info;   ?></div>
<?PHP } ?>
...
<span><?PHP echo $RFP->comp_name;  ?></span><br />
<?php } ?>
...
<div class="lineitem_pan" style="background-color:#F7F7F7; width:658px;">
...
?>
<!--</div>-->
<div class="" style="background-color:#F7F7F7; width:658px;">
...
<span class="grey_12" style="color:#FF0000; float: left;" title="This is modified Line Item">Line Item #<?PHP echo $i+1; ?>:
</span>
<?PHP } else { ?>
<span class="blue_small"><strong>Line Item #<?PHP echo $i+1; ?>:
</strong></span>
<?PHP }  ?>
<div class="lineitem_pan"><input type="text" readonly="readonly"  style="width: 653px; background-color:#F7F7F7; color:#7D7D7D; border:1px solid #D7D7D7;" class="other_tfield"  value="<?PHP echo $TASKS[$i]->linetaskname; ?>"> 
<?php /*?><input type="text" readonly="readonly"  style="width: 311px; background-color:#F7F7F7; color:#7D7D7D; border:1px solid #D7D7D7; " class="other_tfield" value="<?PHP echo $TASKS[$i]->rfpreference; ?>"><?php */?>
<p style="height:10px;"> </p>
<p style="background-color:#F7F7F7; padding:5px; margin:0px; border:1px solid #CDCDCD; text-align:left"><?PHP 
...
<div class="clear"></div>-->
<!-- eof line item pan -->
<?PHP } ?></table><p style="height:10px;"> </p>

<span class="blue_small"><strong>Response to Line Item:</strong></span><br/>
<textarea name="NOTES_<?php echo $TASKS[$i]->task_id; ?>"  class="textarea" rel="editor" id='notes<?php echo $i+1; ?>' style=" width:649px; height:120px;" >&nbsp;<?PHP 
$html = html_entity_decode($TASKS[$i]->Notes) ;
...
<!--<a href="javascript:addEvent2('<?php echo $i+1; ?>','<?php echo $TASKS[$i]->task_id; ?>');"><img alt="Upload another attachment" src="<?php echo Juri::base(); ?>components/com_camassistant/skin/images/uploadanother1.gif"></a>-->
...
<a href="index2.php?option=com_camassistant&controller=proposals&task=downloadfile&title=<?PHP echo $TASKS[$i]->uploadfile_title[$f]; ?>&path=<?PHP echo $TASKS[$i]->uploaded_file[$f]; ?>"><img src="templates/camassistant_left/images/green.png" alt="download file" align="absmiddle" /></a></td><td><?PHP if($rebid != 's') {?><a href="index.php?option=com_camassistant&controller=proposals&spl_req=No&task=Remove_downloadfile&Alt_Prp=<?PHP echo $is_Alt; ?>&prop_id=<?PHP echo $Proposal_id; ?>&vendor_id=<?PHP echo $user->id; ?>&rfp_id=<?PHP echo $TASKS[$i]->rfp_id;  ?>&task_id=<?PHP echo $TASKS[$i]->task_id ?>&doc_id=<?PHP echo $TASKS[$i]->uploaded_doc_ids[$f];  ?>&act=<?PHP echo $act; ?>&rebid=<?PHP echo $rebid; ?>&Itemid=<?PHP echo $_REQUEST['Itemid']; ?>"><img src="templates/camassistant_left/images/red.png" alt="remove file" align="absmiddle" /></a><?PHP } ?></td><td>
...
<a href="index2.php?option=com_camassistant&controller=proposals&task=downloadfile&title=<?PHP echo $TASKS_DOCS[$T]->title; ?>&path=<?PHP echo $TASKS_DOCS[$T]->taskuploads; ?>"><img src="templates/camassistant_left/images/downloadfile.gif" alt="download file" align="absmiddle" /></a> <strong><?PHP echo $TASKS_DOCS[$T]->title;?></strong>
</div>
<!-- eof line item pan -->
<?PHP } } ?>
<div class="clear"></div><?php */?>
<!--end of line item files -->
...
<a href="index2.php?option=com_camassistant&controller=proposals&task=downloadfile&title=<?PHP echo $DOCS[$j]->title; ?>&path=<?PHP echo $file_link; ?>"><img src="templates/camassistant_left/images/green.png" alt="download file" align="absmiddle" /></a> <strong><?PHP echo $DOCS[$j]->upload_doc;?></strong><strong><?PHP echo $DOCS[$j]->title;?></strong>
...
<label><img align="top" height="22" width="145" style="margin: 0px; padding: 0px;" alt="line item pricing" src="templates/camassistant_left/images/Additional_Item_Pricing.gif"> $&nbsp;</label>
...
<script language="JavaScript" type="text/javascript" src="components/com_camassistant/assets/wysiwyg.js"></script>
<textarea id="snotesmain" name="SNOTES" rel="editor" class="textarea" style=" width:649px; height:100px;" >&nbsp;<?PHP /*echo nl2br($SPL_REQ_TABS[6]->Notes);*/ echo $SPL_REQ_TABS[6]->Notes; ?></textarea>
<script language="javascript1.2">
generate_wysiwyg('snotesmain');
</script>
<div style="margin-top:12px; margin-bottom:-22px;" id="dotshead_green" class="green-heading"></div>
<span style="float:left; padding-top:4px;"><span id="upload_file"> </span><a href="javascript:addEvent2_general('1','');" class="uploadfile_editprps"></a>
<div id="newdiv_general1"></div>
	<input name="hidden2" type="hidden" id="theValues_general" value="0">
	<input name="hidden" type="hidden" id="idvals_general" value="0">
	</span><div class="clear"></div>
<?php
//echo "<pre>"; print_r($this->generaldocs); echo "</pre>";
$generaldocs = $this->generaldocs ;
for( $g=0; $g<count($generaldocs); $g++ ){ 
	if( $generaldocs[$g]->filename ) {
?>
	<table class="gendos_<?PHP echo $generaldocs[$g]->id;  ?>"><tr><td><a href="index2.php?option=com_camassistant&controller=proposals&task=downloadfile&title=<?PHP echo $generaldocs[$g]->filename; ?>&path=<?PHP echo $generaldocs[$g]->filepath; ?>"><img src="templates/camassistant_left/images/green.png" vspace="2px"  alt="download file" align="absmiddle" /></a> </td>
	<td><?PHP if($rebid != 's') {?><a href="javascript:void(0);" onclick="deletegeneralfiles(<?PHP echo $Proposal_id; ?>,'<?PHP echo $generaldocs[$g]->proposal_type; ?>',<?PHP echo $generaldocs[$g]->rfpid;  ?>,<?PHP echo $generaldocs[$g]->id;  ?>);"><img src="templates/camassistant_left/images/red.png" alt="remove file" align="absmiddle" /></a><?PHP } ?></td>
      <td><?PHP echo $generaldocs[$g]->filename; ?></td></tr></table>
<?php }
}
?>	
...
<div style="color:green; font-weight: bold; font-size: 16px; float: left; padding-top: 4px;">$</div>&nbsp;<input name="total_lineitem_pricing" id="total_lineitem_pricing" type="text"  value="<?PHP echo $PRICE[0]->tasks_total_price//echo number_format($TOT,2); //$PRICE[0]->tasks_total_price; ?>" readonly="readonly" style="color:black;" />
</div>
<?PHP } ?>
