...
case 'checkboxGroup':
			{
				...
				$aux = str_replace("\r","",$aux);
				$items = explode("\n",$aux);
				if($data['FLOW']=='VERTICAL') $out .= '<table cellspacing="0" cellpading="0">';
				else
				$out .= '<table cellspacing="0" cellpading="0"><tr>';
				foreach($items as $item)
				{
					$buf = explode('|',$item);
					...
					if (!empty($value[$data['NAME']]) && array_search($option_value_trimmed,$value[$data['NAME']]) !== false)
						$option_checked = true;
					if($data['FLOW']=='VERTICAL') $out .= '<tr>';	
					$out .= '<td valign="top"><input '.($option_checked ? 'checked="checked"' : '').' name="form['.$data['NAME'].'][]" type="checkbox" value="'.$option_value_trimmed.'" id="'.$data['NAME'].$i.'" '.$data['ADDITIONALATTRIBUTES'].' /></td><td>';
					$out .='<label for="'.$data['NAME'].$i.'">'.$option_shown_trimmed.'</label></td>';
					if($data['FLOW']=='VERTICAL') $out .='</tr>';
	...
	function RSshowForm($formId,$val='',$validation='')
	{
		...
		$RSadapter->addHeadTag( _RSFORM_FRONTEND_REL_PATH . '/assets/js/customJS.js','js', $GLOBALS['ismodule'] );
