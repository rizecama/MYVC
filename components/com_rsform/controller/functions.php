...
defined( '_JEXEC' ) or die( 'Restricted access' );
	...
	function RSpreviewComponent($formId, $componentId)
	{
		...
		switch($r['ComponentTypeName'])
		{
			...
			case 'startLineItemTask':
			case 'endLineItemTask':
			{
				$defaultValue = RSisCode($data['DEFAULTVALUE']);
				$out.='<td>&nbsp;</td>';
				$out.='<td>{'.$r['ComponentTypeName'].'}</td>';
			}
			break; // added on 07-21-2011 
	...
	function RSfrontComponentBody($formId,$componentId,$data,$value='')
	{
		...
		switch(RSresolveComponentTypeId($r['ComponentTypeId']))
		{
			...
			{
				$defaultValue = RSisCode($data['DEFAULTVALUE']);
				$frequency=str_replace('Linetask','',$data['NAME']);
				//$out .= '<div><div style="float:left;width:145px;margin-left:0px;">Frequency:<input type="text" size="5"  value=""  name="form[fr'.$frequency.']" id="fr'.$frequency.'" /></div>';
//$out .= '<div style="float:left;width:250px;margin-top:3px;"> time(s) per&nbsp;&nbsp;<select class="frsel'.$frequency.'" id="frsel'.$frequency.'" name="form[frsel'.$frequency.'][]"><option value="PLEASE SELECT">Please Select</option><option value="week">week</option><option value="month">month</option><option value="year">year</option><option value="scheduled visit">scheduled visit</option></select>&nbsp;&nbsp;&nbsp;&nbsp;(OR) </div>';
//$out .= '<div>Enter custom Frequency Below:<br/><textarea name="form[frother'.$frequency.']" id="froher'.$frequency.'"  cols="30" style="background:none; height:35px;margin-top:0px;margin-left:2px;"></textarea></div></div>';
				$out .= '<input type="'.RSresolveComponentTypeId($r['ComponentTypeId']).'"  value="'.$defaultValue.'"  name="form['.$data['NAME'].']"	id="'.$data['NAME'].'"  style="display:none;"/>';
				

				
				// code added to show the custom component on front_end -- by saikanth on 07-07-2011
			}
			break; 
 ...
 case 'checkboxGroup':
			{
				...
				foreach($items as $item)
				{
					...
					if($data['FLOW']=='VERTICAL' && $i<count($items)-1) $out.='<br/>';
					$i++;
				}
 ...
 K('#txtcal".$calendars[$componentId]."').datepicker({
...
dateFormat: 'mm-dd-yy',minDate: 'new',changeYear: true,changeMonth:true
			...
			case 'hidden':
			{
				//echo"hai".$defaultValue = RSisCode($data['DEFAULTVALUE']);
	...
	function RSshowForm($formId,$val='',$validation='')
	{
		...
		$RSadapter->addHeadTag( _RSFORM_FRONTEND_REL_PATH . '/assets/js/JSrsform.js','js', $GLOBALS['ismodule'] );
         $RSadapter->addHeadTag( Juri::root() . '/components/com_camassistant/skin1/css/jquery1.css','css', $GLOBALS['ismodule'] );
		 $RSadapter->addHeadTag( Juri::root() . '/components/com_camassistant/skin/js/jquery-1.4.4.min.js','js', $GLOBALS['ismodule'] );
		$RSadapter->addHeadTag( Juri::root() . '/components/com_camassistant/skin/js/jquery-ui-1.8.6.custom.min.js','js', $GLOBALS['ismodule'] );
         $RSadapter->addHeadTag( Juri::root() . '/components/com_camassistant/skin/js/jquery-ui-timepicker-addon.js','js', $GLOBALS['ismodule'] );

$RSadapter->addHeadTag('<script type="text/javascript">
		window.addEvent(\'domready\', function() {SqueezeBox.initialize({});$$(\'input.modal\').each(function(el) {el.addEvent(\'click\', function(e) {new Event(e).stop();SqueezeBox.fromElement(el);});});});</script>');
		...
		$formLayout = '<form method="post" id="userForm" enctype="multipart/form-data" action="'.$u.'"><script type="text/javascript"> K = jQuery.noConflict(); </script>'.$formLayout.'</form>';
...
$linetask1=str_replace('<br/>','',$linetask1);
...
window.parent.document.getElementById("line_taskdesc").value ='<?php echo nl2br($linetask); ?>';
...
///end show cam form 
	...
	function RSprocessForm($formId)
	{
		...
		foreach ($_POST['form'] as $key => $val)
		{
			...
			/************ Varibles Forming **************/
			$val = (is_array($val) ? implode("#splityes#",$val) : $val);
			$key = RScleanVar($key);
			$val = RScleanVar(RSstripjavaVar($val));
			/************ End Varibles Forming **************/
			$lit='Linetask'.$j;
			/****** Data loop **********/
			...
			if(($val)&&($val!='PLEASE SELECT')){
				/********** LIne Caption **********/
				if(!$desc){
				$sql_form = "SELECT * FROM `jos_rsform_properties`
				WHERE `ComponentId` =(SELECT rp.ComponentId
				FROM  jos_rsform_components AS rc, jos_rsform_properties AS rp
				WHERE rc.FormId = '".$_POST['form']['formId']."'
				AND rp.PropertyValue = '".$lit."'
				AND rc.ComponentId = rp.ComponentId
				group by ComponentId  having count(*)>=1)";
				
				
				$db->setQuery($sql_form);
				$requirevendors = $db->loadObjectlist();
				unset($desc);
				foreach($requirevendors as $requirevendor){
				if(($requirevendor->PropertyName=='CAPTION')&&($requirevendor->PropertyValue!='')){
				$linecaption=$requirevendor->PropertyValue;
				}
				if(($requirevendor->PropertyName=='ADDITIONALATTRIBUTES')&&($requirevendor->PropertyValue!='')){
				$addattributes=$requirevendor->PropertyValue;
				}
				if(($requirevendor->PropertyName=='ITEMS')&&($requirevendor->PropertyValue!='')){
				$Items=$requirevendor->PropertyValue;
				}
				if(($requirevendor->PropertyName=='DESCRIPTION')&&($requirevendor->PropertyValue!='')){
				$desc=$requirevendor->PropertyValue;
				}
				}
				
				}
if($val&&$val!=$lit&&$val!='Please Select'){
$linetask .=$val;
echo $val.'<br/>';
$d++;
}
				...
				$val=str_replace(',','#split#',$val);
				$val=str_replace('"','\"',$val);

				/**** Line Task Replace *******/
				$desc = str_replace('{'.$key.'}',$val,$desc);
				 
				/****** End Of Lien Task Replace ********/

				$lit='Linetask'.$j;
				if($key==$lit){

				$linetask=str_replace($lit.':','',$linetask);
				$linetask=str_replace($val,'',$linetask);
				$line_title=$val;
				if($i>1){
				$required_vendor=1;
				}else{
				$db->setQuery("SELECT `line_item_price` FROM `jos_rsform_forms` WHERE`FormId`='".$_POST['form']['formId']."'");
				$db->query();
				$required_vendor=$db->loadResult();
				}
				$line_title=str_replace(':','',$line_title);
				if(strlen($linetask)>3){
				$desc = str_replace('\r\n'," ",$desc);

$lineitem_value=explode(',',$desc);
...
if(($line_desc) || ($d)){
$db->setQuery("INSERT INTO #__rsform_submission_values SET `SubmissionId`='".$SubmissionId."',Frequency='".$frequency."', `FormId`='".$_POST['form']['formId']."',`line_title`='".$line_title."',`required_vendor`='".$required_vendor."', `FieldName`='".$key."', `FieldValue`='".strip_tags($line_desc)."'");
$db->query();
}
unset($d);
			...
			$lit='Linetask'.($j-1);
			$line_title=explode(':',$_POST[form][$lit]);
			$linetask=str_replace($line_title,'',$linetask);
			//echo $linetask;
			if($i>1){
			$required_vendor=1;
			}
			if(strlen($linetask)>3){
			$desc = str_replace('\r\n',' ',$desc);
			$db->setQuery("INSERT INTO #__rsform_submission_values SET `SubmissionId`='".$SubmissionId."',Frequency='".$frequency."', `FormId`='".$_POST['form']['formId']."',`line_title`='".$line_title[1]."',`required_vendor`='".$required_vendor."', `FieldName`='".$key."', `FieldValue`='".strip_tags($desc)."'");
			$db->query();
			}
