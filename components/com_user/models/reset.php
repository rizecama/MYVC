...
//	echo "<pre>"; print_r($questions); exit;
	...
	//completed
	function requestReset($email,$question,$answer)
	{
...
//echo $db->loadResult(); exit;
	...
	//Function to get username using token id
	function getresetusername($token){
		global $mainframe;
		jimport('joomla.user.helper');
		if(strlen($token) != 32) {
			//$this->setError(JText::_('INVALID_TOKEN'));
		...
		$db	= &JFactory::getDBO();
		$db->setQuery('SELECT username, activation FROM #__users WHERE block = 0 and activation!="" ');
		$rows = $db->loadObjectList();
		for( $a=0; $a<count($rows); $a++ ){
			$parts	= explode( ':', $rows[$a]->activation );
			$crypt	= $parts[0];
			$salt	= $parts[1];
			$testcrypt = JUserHelper::getCryptedPassword($token, $salt);
			if ($crypt == $testcrypt)
				{
					$username = $rows[$a]->username ;
				}
		}
		if(  $username  )
		$username = $username ;
	...
	function _sendConfirmationMail($email, $token)
	{
		...
		$uri		= &JFactory::getURI();
		$url		= JURI::base().'index.php?option=com_user&view=reset&layout=confirm&Itemid=137';
		...
		$subject = 'Your request to reset your MyVendorCenter password';
		// Send the e-mail
		$link_reset = "<a href='".$url."'>".$url."</a>";
		$db =& JFactory::getDBO();
		$reset_sql = "SELECT introtext  FROM #__content where id='292'";
		$db->setQuery($reset_sql);
		$body = $db->loadResult();
		$body = str_replace('{token}', $token, $body);
		$body = str_replace('{link}', $link_reset, $body);

		if (!JUtility::sendMail($from, $fromname, $email, $subject, $body, $mode=1))
		{
