...
class UserController extends JController
{
	...
	{ 
		...
		if($from =='adminlogin'){
		$credentials['from'] = JRequest::getVar('from', '', 'method', 'from');
		}
		...
		if(!JError::isError($error))
		{
			...
			$user = JFactory::getUser();
			$db = & JFactory::getDBO();
			if($_REQUEST['username'] != '' && $user->user_type == ''){
$return = 'index.php?option=com_user&view=login&Itemid=137';
}

			if($user->user_type == '11'){
			if($user->subscribe == 'no' || $user->subscribe == ''){
				$return = 'index.php?option=com_camassistant&controller=vendors&task=subscriptions&Itemid=213';		
				}
				else{
				$return = 'index.php?option=com_camassistant&controller=vendors&task=vendor_dashboard&Itemid=112';
				}
			}
			else if($user->user_type == '12'){
			$return = 'index.php?option=com_camassistant&controller=rfpcenter&task=dashboard&Itemid=128';
			}
			else if($user->user_type == '13'){
			$return = 'index.php?option=com_camassistant&controller=rfpcenter&task=dashboard&Itemid=125';
			}
			...
			/***********code for redirecting to vendor billing form upon user first login--- by lalitha**********/
			/*$user = JFactory::getUser();
			$db = & JFactory::getDBO();*/
			/*if($user->user_type == '11')
			{
			...
			if($is_userid != 'paid')
			$return = 'index.php?option=com_camassistant&controller=vendors&task=vendor_billing_form&Itemid=140';
			else
			$return = 'index.php?option=com_camassistant&controller=vendors&task=vendor_dashboard&Itemid=112'; 
			}  */
		...
		/*else {
			...
			/*******end if code for redirecting to vendor billing form upon user first login--- by lalitha**********/
			//echo $return; exit;
	...
	function logout()
	{
		...
		$_COOKIE["nonc_cookie"] = '';
		setcookie("nonc_cookie", "");
		$_COOKIE["recs_cookie"] = '';
		setcookie("recs_cookie", "");
		$_COOKIE["invites_cookie"] = '';
		setcookie("invites_cookie", "");
		$_COOKIE["invitation_cookie"] = '';
		setcookie("invitation_cookie", "");
...
//echo $email; exit;
		...
		if ($model->requestReset($email) === false)
		{
			...
			$this->setRedirect('index.php?option=com_user&view=reset&Itemid=137', $message);
			return false;
		}

		$this->setRedirect('index.php?option=com_user&view=reset&layout=confirm&Itemid=137');
	}
	...
	function confirmreset()
	{
		...
		// Get the model
		$model = &$this->getModel('Reset');
		$username = $model->getresetusername($token);
		// Verify the token
		if ($model->confirmReset($token, $username) !== true)
		{
			$message = JText::sprintf('PASSWORD_RESET_CONFIRMATION_FAILED', $model->getError());
			$this->setRedirect('index.php?option=com_user&view=reset&layout=confirm&Itemid=137', $message);
			return false;
		}
		$this->setRedirect('index.php?option=com_user&view=reset&layout=complete&Itemid=137');
	}
	...
	function completereset()
	{
		...
		if ($model->completeReset($password1, $password2) === false)
		{
			...
			$this->setRedirect('index.php?option=com_user&view=reset&layout=complete&Itemid=137', $message);
			return false;
		}

		$message = JText::_('PASSWORD_RESET_SUCCESS');
		$this->setRedirect('index.php?option=com_user&view=login&Itemid=137', $message);
	}
	...
	function remindusername()
	{
		...
		if ($model->remindUsername($email) === false)
		{
			...
			$this->setRedirect('index.php?option=com_user&view=remind&Itemid=137', $message);
			return false;
		}
