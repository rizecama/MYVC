 ...
 **********************************************
 ...
 $Id: iCalcreator.class.php 708 2011-03-01 01:07:40Z jeffchannell $
...
/*         A little setup                                                        */
/*********************************************************************************/
define( 'ICALCREATOR_VERSION', 'jCal Pro 2.2.14.578' );
/*********************************************************************************/
/*********************************************************************************/
/**
 * vcalendar class
 ...
 */
	function parse( $filename=FALSE ) {
		...
		elseif (preg_match($remoteReg, $filename)) {
						/* remote file - URL */
			$this->setConfig( 'URL', $filename );
			if( !$filename = $this->getConfig( 'url' )) {
				$app->enqueuemessage('url didn\'t get set in config');
				return FALSE;                 /* err 2 */
			}
		}
		else {
			...
			if( !empty( $parts['dirname'] ) && ( '.' != $parts['dirname'] )) {
				if( !$this->setConfig( 'directory', $parts['dirname'] )) {
					$app->enqueuemessage('local file directory could not be set');
					return FALSE;               /* err 3 */
				}
			}
			if( !$this->setConfig( 'filename', $parts['basename'] )) {
				$app->enqueuemessage('local file name could not be set');
				return FALSE;                 /* err 4 */
			}
		}
		if( !preg_match($remoteReg, $filename)) {
						/* local file error tests */
			if( !is_file( $filename )) {      /* err 5 */
				$app->enqueuemessage('filename is not a file');
				return FALSE;
			}
			if( !is_readable( $filename )) {
				$app->enqueuemessage('file is not readable');
				return FALSE;                 /* err 6 */
			}
			if( !filesize( $filename )) {
				$app->enqueuemessage('file is empty');
				return FALSE;                 /* err 7 */
			}
			clearstatcache();
		}
		/* READ FILE */
		/* NOTE: the checks below are used in case allow_url_fopen is disabled and the ics is remote */
		if (preg_match($remoteReg, $filename)) {
			if ( function_exists( 'curl_version' ) ) {
				...
				if( FALSE === ( $handle = @fopen( $filename, 'r' ) ) ) {
					$app->enqueuemessage('fopen could not open url');
					return FALSE;                   /* err 1 */
				}
				while ( !feof( $handle ) ) {
					$rows[] = fgets( $handle, 4096 );
				}
			   fclose( $handle );
			} else {
				$app->enqueuemessage('no method to fetch url');
				return FALSE;
			}
		} else {
			/* USE FILE */
			if( FALSE === ( $rows = file( $filename ) ) ) {
				$app->enqueuemessage('could not use file() to open url');
				return FALSE;                   /* err 1 */
			}
		}
						/* identify BEGIN:VCALENDAR, MUST be first row */
		if( 'BEGIN:VCALENDAR' != strtoupper( trim( $rows[0] ))) {
			$app->enqueuemessage('improper header');
			return FALSE;                   /* err 8 */
		}
		...
		if( 'END:VCALENDAR'   != strtoupper( trim( $rows[count( $rows ) - 1] ))) {
			$app->enqueuemessage('improper close');
			return FALSE;                   /* err 9 */
		}
		if( 3 > count( $rows )) {
			$app->enqueuemessage('not enough data');
			return FALSE;                   /* err 10 */
		}
		$comp    = $subcomp = null;
		$actcomp = & $this;
		...
		foreach( $rows as $line ) {
			...
			elseif( 1 != $calsync ) {
				$app->enqueuemessage('could not sync row');
				return FALSE;                 /* err 20 */
			}
				...
				if( null != $comp ) {
					...
					switch( $compname ) {
						...
						case 'VTIMEZONE':
							...
							break;
						default:
							$app->enqueuemessage('unknown type');
							return FALSE; // err 7
							break;
					} // end - switch
					$actcomp = & $comp;
