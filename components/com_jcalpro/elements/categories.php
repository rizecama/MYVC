...
class JElementCategories extends JElement
{
	...
	*/
	...
	{
		$db =& JFactory::getDBO();
		$sql = "SHOW tables like '".$db->getPrefix()."jcalpro2_categories'";
		$db->setQuery( $sql );
		$tables = $db->loadObjectList();

		if ( !count( $tables ) ) {
			return 'JCal Pro category table not found in database...';
		}
		...
		$sql = "SELECT cat_id id, cat_parent as parent, cat_name as name FROM #__jcalpro2_categories WHERE ".$where." ORDER BY name";
		$db->setQuery( $sql );
