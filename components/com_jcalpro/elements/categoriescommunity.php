...
/**
 * CategoriesCommunity Element
 */
class JElementCategoriesCommunity extends JElement
{
	/**
	* Element name
	*
	...
	* @var		string
	*/
	var	$_name = 'CategoriesCommunity';

	function fetchElement( $name, $value, &$node, $control_name)
	{
		if ( !file_exists( JPATH_ADMINISTRATOR.DS.'components'.DS.'com_community'.DS.'admin.community.php' ) ) {
			return 'JomSocial is not installed...';
		}

		$db =& JFactory::getDBO();
		$sql = "SHOW tables like '".$db->getPrefix()."community_events_category'";
		$db->setQuery( $sql );
		$tables = $db->loadObjectList();

		if ( !count( $tables ) ) {
			return 'JomSocial category table not found in database...';
		}
		...
		$sql = "SELECT id, 0 as parent, name FROM #__community_events_category ORDER BY name";
		$db->setQuery( $sql );
