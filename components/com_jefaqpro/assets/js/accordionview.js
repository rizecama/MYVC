...
function updateHits( order_id )
{
	...
	var url=path+'index.php?option=com_jefaqpro&view=faq&task=hits&faqid='+id+'&timeNow'+timeNow;
	ajaxRequest.open("GET", url, true);
	ajaxRequest.send(null);

}

function getResponse( i,response, faqid, catid, user )
{
	...
	if ( user == '1') {
		document.getElementById('je-userlogin'+i).innerHTML = document.getElementById('user_login').value;
		return false;
	}

	if ( user == '2') {
		document.getElementById('je-userlogin'+i).innerHTML = document.getElementById('je_already_added').value;
		return false;
	}
	...
	var path = document.getElementById('path').value;
	var timeNow = Math.floor(Math.random()*11);
	var url=path+'index.php?option=com_jefaqpro&view=faq&task=response&response='+res+'&faqid='+faqid+'&catid='+catid+'&ival='+i+'&timeNow'+timeNow;
	ajaxRequest.open("GET", url, true);
	ajaxRequest.send(null);
