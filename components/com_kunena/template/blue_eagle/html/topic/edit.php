...
<form action="<?php echo KunenaRoute::_('index.php?option=com_kunena') ?>" method="post" class="postform form-validate" id="postform" name="postform" enctype="multipart/form-data" onsubmit="return myValidate(this);">
	...
	<?php endif; ?>
	<input type="hidden" class="forum_type" name="type" value="<?php echo $_REQUEST['type']; ?>" />
...
<div class="kblock">
	...
	<div class="kcontainer">
		<div class="kbody">
<table class="kblocktable<?php echo !empty ( $this->category->class_sfx ) ? ' kblocktable' . $this->escape($this->category->class_sfx) : ''?>" id="kpostmessage" style="margin-left:10px;">
	<tbody id="kpost-message">
		<?php if (isset($this->selectcatlist)): ?>
		<?php /*?><tr id="kpost-category" class="krow<?php echo 1 + $this->k^=1 ?>">
		...
		<tr class="krow<?php echo 1 + $this->k^=1 ?>" id="kanynomous-check" <?php if (!$this->category->allow_anonymous): ?>style="display:none;"<?php endif; ?>>
			<td class="kcol-first" style="width:17%">
		...
		<?php endif; ?>
		<?php if($this->escape($this->message->subject)){
		$display = 'none';
		}
		else{
		$display = '';
		...
		?>
		<br />
		<tr id="kpost-subject" class="krow<?php echo 1 + $this->k^=1 ?>" style="display:<?php echo $display; ?>">
			<td class="kcol-first" style="width:17%">
		...
		<?php /*?><tr id="kpost-topicicons" class="krow<?php echo 1 + $this->k^=1 ?>">
			<td class="kcol-first">
...
<script type="text/javascript" src="https://ajax.googleapis.com/ajax/libs/jquery/1.5.1/jquery.min.js"></script>	
						...
						G = jQuery.noConflict();	
						G(function(){
						G(":file").change(function(){
						var filename = G(":file").val().replace(/.*(\/|\\)/, '');
							G('.kfile-input-textbox').val(filename);
						 });
						 });
...
<p align="right">
<input type="submit" name="ksubmit" class="kbutton" id="submittopic"
				value="<?php //echo (' ' . JText::_('COM_KUNENA_SUBMIT') . ' ');?>"
				title="<?php echo (JText::_('COM_KUNENA_EDITOR_HELPLINE_SUBMIT'));?>" tabindex="4" style="background:url('templates/camassistant_inner/images/SubmitShort.png') no-repeat; height:47px; width:117px; border:none; cursor:pointer;"  />
<input type="button" name="cancel" class="kbutton" id="canceltopic"
				value="<?php //echo (' ' . JText::_('COM_KUNENA_CANCEL') . ' ');?>"
				onclick="javascript:window.history.back();"
				title="<?php echo (JText::_('COM_KUNENA_EDITOR_HELPLINE_CANCEL'));?>" tabindex="5" style="background:url('templates/camassistant_inner/images/CancelShort.png') no-repeat; height:47px; width:116px; border:none; cursor:pointer;"  />				
</p>
<?php 
if($_REQUEST['id']){
...
?>
<p style="height:40px; <?php echo $border; ?>"></p>
<?php
