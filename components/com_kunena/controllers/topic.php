...
class KunenaControllerTopic extends KunenaController {
	...
	//echo "<pre>"; print_r($_REQUEST); exit;
		...
		$topic_type = JRequest::getVar('type','') ;
		$fields = array (
			'catid' => $this->catid,
		...
		if (!$this->id) {
			...
			$topic->type = 'personal';
			$topic->selectedid = $topic_type;
			$message->selectedid = $topic_type;
			}
			else{
			$message->type = '';
			$topic->type = '';
		...
		$url_return = 'index.php?option=com_kunena&view=category&catid=2&Itemid=232&type='.$topic->selectedid.'';
