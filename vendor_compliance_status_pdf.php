...
echo "Reminder: ".$reminder.'<br />'; 
  ...
  <tr><td><img src="/var/www/vhosts/vps61048.vps.ovh.ca/httpdocs/templates/camassistant_left/images/myvc_status.jpg" />';
  ...
  $only_pdfcontent .= '</td></tr>
		...
		foreach($vendordata_pdf as $key=>$value){
			...
			for($last=0; $last<count($value); $last++){
				...
				$only_pdfcontent .= '</tr>';
				if( $last+1 == $count ){
				$only_pdfcontent .= '<tr><td colspan="12"><img src="/var/www/vhosts/vps61048.vps.ovh.ca/httpdocs/templates/camassistant_left/images/top_border.jpg" /></td></tr>';
				}
		...
		$pdf->lastPage();
		$title = $total_managers[$t]->cust_id.'_compliancereports.pdf'; //set title
		$pdf->SetTitle($title);//set title
		ob_end_clean();
		$pdf->Output('/var/www/vhosts/vps61048.vps.ovh.ca/httpdocs/components/com_camassistant/assets/compliance_reports/' . $title, 'F');
	...
	//Completed 
		...
		$comp_stand = $table_start.$only_content.$table_end ;
	           ...
	           if( $docs_permission->documenttype == 0 )
			   {
				...
				$sub = "Preferred Vendor Compliance Status_devpdf";
