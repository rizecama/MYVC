...
{		
	...
	$duedateandtime = strtotime(date('Y-m-d')) + $duetime ; 
	$thisday = date('Y-m-d H:i');
	$todaydatetime = strtotime($thisday);
	$timedifference = $duedateandtime - $todaydatetime;
	//echo  '<br />Time: ' . $timedifference . '<br />' ; 
	if($timedifference >= '60' && $timedifference <= '360') {
	$rfpid =  $rfps[$r]->id ;
...
//	echo "<pre>"; print_r($user_ids); echo count($user_ids); exit;
	for($u=0;$u<count($user_ids);$u++)
	{
		...
		//To get the body
		$message = "SELECT introtext FROM #__content WHERE  id=227";
		$db->setQuery($message);
		$body = $db->loadResult();
//Completed		
...
//To get the vendor company name
		$vendorcompany = "SELECT company_name FROM #__cam_vendor_company WHERE user_id='".$user_ids[$u]->proposedvendorid."'";
		$db->setQuery($vendorcompany);
...
//Completed		
		...
		$body = str_replace('{VENDORNAME}',$vendor_company,$body);
	...
	} //condition closed for checking the time to before 8 days
}
